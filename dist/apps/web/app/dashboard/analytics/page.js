...
const BarChart = ({ data, title, color = '#3b82f6' }) => {
    if (!data?.labels?.length) {
        return (<div style={{ textAlign: 'center', padding: '2rem', color: '#6b7280' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
        <div style={{ position: 'relative', width: '200px', height: '200px' }}>
          <svg width="200" height="200" style={{ transform: 'rotate(-90deg)' }}>
            ...
            currentAngle += angle;
            return (<path key={index} d={pathData} fill={color} stroke="white" strokeWidth="2" style={{ cursor: 'pointer' }}>
        ...
        <div style={{ flex: 1 }}>
          {data.labels.map((label, index) => {
            ...
            const color = data.backgroundColor[index] || `hsl(${(index * 137.5) % 360}, 70%, 60%)`;
            return (<div key={index} style={{ display: 'flex', alignItems: 'center', marginBottom: '0.5rem' }}>
                <div style={{
                    width: '16px',
                    height: '16px',
                    backgroundColor: color,
    ...
    return (<div style={{
      ...
      <div style={{ position: 'relative', width: '120px', height: '60px', margin: '0 auto 1rem' }}>
        <svg width="120" height="60" style={{ overflow: 'visible' }}>
          {/* Fondo del gauge */}
          <path d="M 10 50 A 50 50 0 0 1 110 50" fill="none" stroke="#e5e7eb" strokeWidth="8"/>
          {/* Progreso del gauge */}
          <path d="M 10 50 A 50 50 0 0 1 110 50" fill="none" stroke={color} strokeWidth="8" strokeDasharray={`${(percentage / 100) * 157} 157`} style={{ transition: 'stroke-dasharray 1s ease' }}/>
        </svg>
        <div style={{
            position: 'absolute',
            bottom: '10px',
...
function AnalyticsPage() {
    const [ventasTiempo, setVentasTiempo] = (0, react_1.useState)(null);
    const [deudasClientes, setDeudasClientes] = (0, react_1.useState)(null);
    const [deudasProveedores, setDeudasProveedores] = (0, react_1.useState)(null);
    const [ventasCategoria, setVentasCategoria] = (0, react_1.useState)(null);
    const [kpisVisuales, setKpisVisuales] = (0, react_1.useState)(null);
    const [periodo, setPeriodo] = (0, react_1.useState)('mensual');
    ...
    const cargarDatos = async () => {
        try {
            ...
            const [ventasResponse, deudasClientesResponse, deudasProveedoresResponse, ventasCategoriaResponse, kpisResponse] = await Promise.all([
    ...
    return (<div className="dashboard-container">
      ...
      {/* KPIs Visuales */}
      {kpisVisuales && (<div className="activity-card" style={{ marginBottom: '2rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '2rem' }}>
        ...
        <div className="activity-card">
          <BarChart data={ventasTiempo ? {
            labels: ventasTiempo.labels,
            data: ventasTiempo.datasets?.[0]?.data || []
        } : null} title="📊 Evolución de Ventas" color="#3b82f6"/>
          {ventasTiempo?.totales && (<div style={{ marginTop: '1rem', padding: '1rem', backgroundColor: '#f8fafc', borderRadius: '8px' }}>
        ...
        <div className="activity-card">
          <PieChart data={ventasCategoria?.graficoPie} title="🏷️ Ventas por Categoría"/>
        </div>
        ...
        <div className="activity-card">
          ...
          {deudasClientes?.graficoEdadSaldos && (<BarChart data={{
                labels: deudasClientes.graficoEdadSaldos.labels || [],
                data: deudasClientes.graficoEdadSaldos.data || []
            }} title="" color="#10b981"/>)}
          {deudasClientes?.totales && (<div style={{ marginTop: '1rem', padding: '1rem', backgroundColor: '#ecfdf5', borderRadius: '8px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem' }}>
        ...
        <div className="activity-card">
          ...
          {deudasProveedores?.graficoEdadSaldos && (<BarChart data={{
                labels: deudasProveedores.graficoEdadSaldos.labels || [],
                data: deudasProveedores.graficoEdadSaldos.data || []
            }} title="" color="#ef4444"/>)}
          {deudasProveedores?.totales && (<div style={{ marginTop: '1rem', padding: '1rem', backgroundColor: '#fef2f2', borderRadius: '8px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem' }}>
