...
const PagosPage = () => {
    const [pagos, setPagos] = (0, react_1.useState)([]);
    ...
    const [filtroPeriodo, setFiltroPeriodo] = (0, react_1.useState)('todos');
    ...
    const loadData = async () => {
        try {
            setLoading(true);
            // Cargar pagos
            const pagosData = await api.get('/api/rrhh/pagos');
    ...
    const generarComprobante = async (pagoId) => {
        try {
    ...
    return (<div className="dashboard-container">
      ...
      <div style={{
        ...
        <div>
          ...
          <select value={filtroPeriodo} onChange={(e) => setFiltroPeriodo(e.target.value)} className="form-control" style={{ width: '150px' }}>
            <option value="todos">Todos</option>
            {getPeriodosUnicos().map(periodo => (<option key={periodo} value={periodo}>{periodo}</option>))}
          </select>
        </div>
      </div>
      ...
      <div className="table-container">
        ...
        <div className="table-wrapper">
          <table className="data-table">
            ...
            <tbody>
              {filtrarPagos().map((pago) => (<tr key={pago.id}>
                  <td>
                    <div>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      {pago.estado === 'pendiente' && (<button onClick={() => procesarPago(pago.id)} className="action-btn bg-green-500 hover:bg-green-600 text-white" title="Procesar Pago">
                          💰 Pagar
                        </button>)}
                      {pago.estado === 'procesado' && (<button onClick={() => generarComprobante(pago.id)} className="action-btn bg-blue-500 hover:bg-blue-600 text-white" title="Descargar Comprobante">
                          📄 Comprobante
                        </button>)}
                      <button onClick={() => {
                const detalles = `
...
MONTO NETO: S/ ${(pago.monto_neto || 0).toLocaleString()}
          ...
          {filtrarPagos().length === 0 && (<div style={{
                textAlign: 'center',
                padding: '2rem',
                color: '#6b7280'
            }}>
        ...
        <div className="table-wrapper">
          <table className="data-table">
            ...
            return (<tr key={empleado.id}>
                    ...
                    <td>
                      {ultimoPago && (<span className={`px-2 py-1 rounded text-xs font-medium ${getEstadoColor(ultimoPago.estado)}`}>
