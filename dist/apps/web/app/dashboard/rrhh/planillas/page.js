...
const PlanillaModal_1 = __importDefault(require("@/components/modals/PlanillaModal"));
const use_api_1 = require("@/hooks/use-api");
const PlanillasPage = () => {
    const { get } = (0, use_api_1.useApi)();
    const [planillas, setPlanillas] = (0, react_1.useState)([]);
    const [loading, setLoading] = (0, react_1.useState)(true);
    const [detallePlanilla, setDetallePlanilla] = (0, react_1.useState)([]);
    const [showDetalle, setShowDetalle] = (0, react_1.useState)(false);
    const [showPlanillaModal, setShowPlanillaModal] = (0, react_1.useState)(false);
    ...
    const verDetallePlanilla = async (planillaId) => {
        try {
            setLoading(true);
    ...
    const generarReporteProfesional = async (planillaId, periodo) => {
        try {
            const response = await fetch(`http://localhost:3001/api/rrhh/planillas/${planillaId}/detalle`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
    ...
    const generarReporteHTML = (empleados, periodo) => {
    ...
    const aprobarPlanilla = async (planillaId) => {
    ...
    const descargarBoleta = async (empleadoPlanillaId) => {
        try {
    ...
    const planillasCalculadas = planillasArray.filter((p) => p?.estado === 'calculada' || p?.estado === 'aprobada');
    const totalNomina = planillasCalculadas.reduce((sum, p) => sum + (parseFloat(p?.total_neto) || 0), 0);
    const planillasEnProceso = planillasArray.filter((p) => p?.estado === 'borrador').length;
    return (<div className="dashboard-container">
      {/* Header */}
      <div className="dashboard-header">
        ...
        <button className="refresh-btn" onClick={abrirModalPlanilla}>
          <span>🚀</span>
      ...
      <div className="activity-section">
        ...
        <div className="activity-card">
          {planillasArray.length === 0 ? (<div className="activity-empty">
              ...
              <button className="btn btn-primary" onClick={abrirModalPlanilla}>
              ...
              <table>
                ...
                <tbody>
                  {planillasArray.map((planilla) => (<tr key={planilla?.id || 'unknown'}>
                      <td>
                        <div>
                      ...
                      <td>
                        <span className={planilla?.estado === 'borrador' ? 'status-warning' :
                    ...
                    fontWeight: 'bold'
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
                          ...
                          {planilla?.estado === 'borrador' && (<button style={{
                        padding: '4px 8px',
                        fontSize: '0.7rem',
                        fontWeight: '600',
                        ...
                        gap: '4px',
                        transition: 'all 0.2s'
                    }} onClick={() => editarPlanilla(planilla)} title="Editar planilla" onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#2563eb'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#3b82f6'}>
                              ✏️ Editar
                            </button>)}
                    ...
                    gap: '4px',
                    transition: 'all 0.2s'
                }} onClick={() => verDetallePlanilla(planilla?.id)} title="Ver detalle completo" onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#4b5563'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#6b7280'}>
                            👁️ Ver
                          </button>
                    ...
                    gap: '4px',
                    transition: 'all 0.2s'
                }} onClick={() => generarReporteProfesional(planilla?.id, planilla?.periodo)} title="Generar reporte profesional" onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#6d28d9'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#7c3aed'}>
                            📊 Reporte
                          </button>
                          
                          {/* Botón Aprobar - Solo para calculadas */}
                          {planilla?.estado === 'calculada' && (<button style={{
                        padding: '4px 8px',
                        fontSize: '0.7rem',
                        fontWeight: '600',
                        ...
                        gap: '4px',
                        transition: 'all 0.2s'
                    }} onClick={() => aprobarPlanilla(planilla?.id)} title="Aprobar planilla" onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#047857'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#059669'}>
                              ✅ Aprobar
                            </button>)}
                          
                          {/* Estado Cerrada - Solo visual */}
                          {planilla?.estado === 'aprobada' && (<span style={{
      ...
      {showDetalle && Array.isArray(detallePlanilla) && (<div style={{
                position: 'fixed',
                top: 0,
                left: 0,
                right: 0,
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
              ...
              <button onClick={() => setShowDetalle(false)} style={{
                background: 'none',
                border: 'none',
                fontSize: '1.5rem',
                cursor: 'pointer',
            ...
            {detallePlanilla.length === 0 ? (<div style={{ textAlign: 'center', padding: '2rem' }}>
                ...
                <table style={{ width: '100%', marginBottom: '2rem' }}>
                  ...
                  <tbody>
                    {detallePlanilla.map((empleado) => (<tr key={empleado?.id || 'unknown'}>
                        ...
                        <td style={{ padding: '0.75rem', borderBottom: '1px solid var(--primary-100)', textAlign: 'center' }}>
                          <button className="btn-icon" onClick={() => descargarBoleta(empleado?.id)} title="Descargar boleta profesional">
                            📄
                          </button>
                        </td>
                      </tr>))}
            ...
            <div style={{
              ...
              <button onClick={() => setShowDetalle(false)} className="btn btn-secondary">
                Cerrar
              </button>
            </div>
          </div>
      ...
      <PlanillaModal_1.default isOpen={showPlanillaModal} onClose={() => {
            console.log('🔥 onClose llamado - cerrando modal');
            setShowPlanillaModal(false);
        }} onSuccess={handlePlanillaSuccess}/>
    </div>);
};
exports.default = PlanillasPage;
