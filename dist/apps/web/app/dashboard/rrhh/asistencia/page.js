...
const AsistenciaPage = () => {
    ...
    const [asistencias, setAsistencias] = (0, react_1.useState)([]);
    const [fecha, setFecha] = (0, react_1.useState)(new Date().toISOString().split('T')[0]);
    ...
    const getAsistenciaEmpleado = (empleadoId) => {
    ...
    return (<div className="dashboard-container">
      ...
      <div className="dashboard-header">
        ...
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          <input type="date" value={fecha} onChange={(e) => setFecha(e.target.value)} className="form-control" style={{ width: 'auto' }}/>
            ...
            return (<tr key={empleado.id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: '0.5rem' }}>
                        {!asistencia?.hora_entrada ? (<button onClick={() => marcarAsistencia(empleado.id, 'entrada')} className="action-btn bg-green-500 hover:bg-green-600 text-white" title="Marcar Entrada">
                            ⏰ Entrada
                          </button>) : !asistencia?.hora_salida ? (<button onClick={() => marcarAsistencia(empleado.id, 'salida')} className="action-btn bg-red-500 hover:bg-red-600 text-white" title="Marcar Salida">
                            🚪 Salida
                          </button>) : (<span className="text-sm text-gray-500">Completo</span>)}
                      </div>
                    </td>
                  </tr>);
        })}
...
exports.default = AsistenciaPage;
