...
const VacanteModal_1 = __importDefault(require("@/components/modals/VacanteModal"));
const use_toast_1 = require("@/components/ui/use-toast");
const CandidatosPage = () => {
    const [candidatos, setCandidatos] = (0, react_1.useState)([]);
    const [vacantes, setVacantes] = (0, react_1.useState)([]);
    const [departamentos, setDepartamentos] = (0, react_1.useState)([]);
    const [filtroEstado, setFiltroEstado] = (0, react_1.useState)('todos');
    const [filtroVacante, setFiltroVacante] = (0, react_1.useState)('todas');
    const [showModal, setShowModal] = (0, react_1.useState)(false);
    const [candidatoEdit, setCandidatoEdit] = (0, react_1.useState)(null);
    ...
    const [showVacanteModal, setShowVacanteModal] = (0, react_1.useState)(false);
    const [vacanteData, setVacanteData] = (0, react_1.useState)({
        titulo: '',
        descripcion: '',
        salario_min: '',
        salario_max: '',
    ...
    return (<div className="dashboard-container">
      ...
      <div className="dashboard-header">
        ...
        <div style={{ display: 'flex', gap: '1rem' }}>
          <button className="refresh-btn" onClick={() => setShowVacanteModal(true)}>
            📋 Nueva Vacante
          </button>
      ...
      <div style={{
        ...
        <div>
          ...
          <select value={filtroEstado} onChange={(e) => setFiltroEstado(e.target.value)} className="form-control" style={{ width: '150px' }}>
        ...
        <div>
          ...
          <select value={filtroVacante} onChange={(e) => setFiltroVacante(e.target.value)} className="form-control" style={{ width: '200px' }}>
      ...
      <div className="table-container">
        ...
        <div className="table-wrapper">
          <table className="data-table">
            ...
            <tbody>
              {filtrarCandidatos().map((candidato) => (<tr key={candidato.id}>
                  <td>
                    <div>
                ...
                setShowModal(true);
            }} className="action-btn bg-blue-500 hover:bg-blue-600 text-white" title="Ver CV">
                        👁️
                      </button>
                      <button onClick={() => {
                setCandidatoEdit(candidato);
                setShowModal(true);
            }} className="action-btn bg-green-500 hover:bg-green-600 text-white" title="Editar">
                        ✏️
                      </button>
                    </div>
                  </td>
          ...
          {filtrarCandidatos().length === 0 && (<div style={{
                textAlign: 'center',
                padding: '2rem',
                color: '#6b7280'
            }}>
      ...
      {showModal && (<div style={{
          ...
          <div style={{
            ...
            <CandidatoFormulario candidato={candidatoEdit} vacantes={vacantes} onSuccess={() => {
                setShowModal(false);
                setCandidatoEdit(null);
                loadData();
            }} onCancel={() => {
      ...
      <VacanteModal_1.default isOpen={showVacanteModal} onClose={() => setShowVacanteModal(false)} onSuccess={loadData} departamentos={departamentos}/>
    </div>);
};
// Componente de formulario simple dentro del mismo archivo
function CandidatoFormulario({ candidato, vacantes, onSuccess, onCancel }) {
    const { toast } = (0, use_toast_1.useToast)();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (formData.experiencia_anos > 0)
                puntuacion += 20;
            if (formData.experiencia_anos >= 3)
                puntuacion += 15;
            if (formData.cv_url)
                puntuacion += 15;
    ...
    return (<form onSubmit={handleSubmit} style={{ padding: '2rem' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
          <div>
            ...
            <input type="text" value={formData.nombres} onChange={(e) => setFormData({ ...formData, nombres: e.target.value })} style={{
            width: '100%',
            padding: '0.75rem',
          ...
          <div>
            ...
            <input type="text" value={formData.apellidos} onChange={(e) => setFormData({ ...formData, apellidos: e.target.value })} style={{
            width: '100%',
            padding: '0.75rem',
            border: '1px solid #d1d5db',
            borderRadius: '6px',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
          <div>
            ...
            borderRadius: '6px',
            outline: 'none'
        }} placeholder="juan.perez@gmail.com" required/>
          </div>
          ...
          <div>
            ...
            <input type="tel" value={formData.telefono} onChange={(e) => setFormData({ ...formData, telefono: e.target.value })} style={{
            width: '100%',
            padding: '0.75rem',
            border: '1px solid #d1d5db',
            borderRadius: '6px',
        ...
        <div>
          ...
          <select value={formData.id_vacante} onChange={(e) => setFormData({ ...formData, id_vacante: e.target.value })} style={{
            width: '100%',
            padding: '0.75rem',
            border: '1px solid #d1d5db',
            borderRadius: '6px',
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
          <div>
            ...
            <input type="number" value={formData.experiencia_anos} onChange={(e) => setFormData({ ...formData, experiencia_anos: parseInt(e.target.value) || 0 })} style={{
            width: '100%',
            padding: '0.75rem',
            border: '1px solid #d1d5db',
            borderRadius: '6px',
          ...
          <div>
            ...
            <input type="number" value={formData.pretension_salarial} onChange={(e) => setFormData({ ...formData, pretension_salarial: parseInt(e.target.value) || 0 })} style={{
            width: '100%',
            padding: '0.75rem',
            border: '1px solid #d1d5db',
            borderRadius: '6px',
          ...
          <div>
            ...
            <select value={formData.estado} onChange={(e) => setFormData({ ...formData, estado: e.target.value })} style={{
            width: '100%',
            padding: '0.75rem',
            border: '1px solid #d1d5db',
            borderRadius: '6px',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          <div>
            ...
            <input type="url" value={formData.cv_url} onChange={(e) => setFormData({ ...formData, cv_url: e.target.value })} style={{
            width: '100%',
            padding: '0.75rem',
            border: '1px solid #d1d5db',
            borderRadius: '6px',
          ...
          <div>
            ...
            <input type="url" value={formData.linkedin_url} onChange={(e) => setFormData({ ...formData, linkedin_url: e.target.value })} style={{
            width: '100%',
            padding: '0.75rem',
            border: '1px solid #d1d5db',
            borderRadius: '6px',
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <textarea value={formData.observaciones} onChange={(e) => setFormData({ ...formData, observaciones: e.target.value })} rows={3} style={{
            width: '100%',
            padding: '0.75rem',
            border: '1px solid #d1d5db',
            borderRadius: '6px',
      ...
      <div style={{
        ...
        <button type="submit" disabled={loading} style={{
          ...
          {loading ? (<>⏳ Guardando...</>) : (<>{candidato?.id ? '✏️ Actualizar' : '➕ Registrar'} Candidato</>)}
        </button>
      </div>
    </form>);
}
exports.default = CandidatosPage;
