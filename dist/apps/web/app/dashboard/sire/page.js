...
exports.default = SIREPage;
const react_1 = require("react");
const use_api_1 = require("@/hooks/use-api");
const SireReportModal_1 = __importDefault(require("@/components/modals/SireReportModal"));
function SIREPage() {
    ...
    const loadReports = async () => {
        ...
        if (filters.tipoReporte)
            queryParams.append('tipoReporte', filters.tipoReporte);
    ...
    const sendToSunat = async (reportId) => {
    ...
    return (<div className="dashboard-container">
      ...
      <div className="activity-section">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem' }}>
            <input type="month" value={filters.periodo} onChange={(e) => setFilters(prev => ({ ...prev, periodo: e.target.value }))} style={{
            padding: '0.5rem 1rem',
            borderRadius: '8px',
            ...
            <select value={filters.tipoReporte} onChange={(e) => setFilters(prev => ({ ...prev, tipoReporte: e.target.value }))} style={{
            padding: '0.5rem 1rem',
            borderRadius: '8px',
            border: '1px solid rgba(255,255,255,0.2)',
            background: 'rgba(255,255,255,0.1)',
        ...
        <div className="activity-card">
          <div style={{ overflow: 'auto' }}>
            ...
            return (<tr style={{ borderBottom: '1px solid rgba(0,0,0,0.05)' }} key={report.id}>
                      ...
                      <td style={{ padding: '1rem' }}>
                        <div>
                          ...
                          {report.filename && (<div style={{ fontSize: '0.8rem', opacity: '0.7' }}>
                              {report.filename}
                            </div>)}
                        </div>
                      </td>
                      ...
                      <td style={{ padding: '1rem', textAlign: 'center' }}>
                        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center' }}>
                          {report.estado === 'GENERADO' && (<>
                              <button onClick={() => downloadReport(report.id, report.filename || 'reporte.txt')} style={{
                        background: 'rgba(59, 130, 246, 0.1)',
                        border: '1px solid rgba(59, 130, 246, 0.2)',
                        padding: '0.5rem 1rem',
                        borderRadius: '6px',
                        ...
                        fontSize: '0.8rem'
                              ...
                              <button onClick={() => sendToSunat(report.id)} style={{
                        ...
                        fontSize: '0.8rem'
                          ...
                          {report.estado === 'ENVIADO' && (<button onClick={() => downloadReport(report.id, report.filename || 'reporte.txt')} style={{
                        background: 'rgba(59, 130, 246, 0.1)',
                        border: '1px solid rgba(59, 130, 246, 0.2)',
                        padding: '0.5rem 1rem',
                        borderRadius: '6px',
                        ...
                        fontSize: '0.8rem'
                          ...
                          {report.estado === 'GENERANDO' && (<span style={{
                        padding: '0.5rem 1rem',
                        fontSize: '0.8rem',
                        color: '#f59e0b'
                    }}>
                          ...
                          {report.estado === 'ERROR' && (<button onClick={() => setIsModalOpen(true)} style={{
            ...
            {reports.length === 0 && !api.loading && (<div style={{ textAlign: 'center', padding: '3rem', color: '#6b7280' }}>
      ...
      {/* SIRE Report Modal */}
      <SireReportModal_1.default isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSuccess={handleReportGenerated}/>
