...
function POSPage() {
    const api = (0, use_api_1.useApi)();
    const supabase = (0, auth_helpers_nextjs_1.createClientComponentClient)();
    // Estados principales
    const [productos, setProductos] = (0, react_1.useState)([]);
    const [carrito, setCarrito] = (0, react_1.useState)([]);
    const [estadoCaja, setEstadoCaja] = (0, react_1.useState)(null);
    const [metodosPago, setMetodosPago] = (0, react_1.useState)([]);
    const [clientes, setClientes] = (0, react_1.useState)([]);
    const [historialVentas, setHistorialVentas] = (0, react_1.useState)([]);
    const [facturaSeleccionada, setFacturaSeleccionada] = (0, react_1.useState)(null);
    // Estados de UI
    const [busqueda, setBusqueda] = (0, react_1.useState)('');
    const [categoriaFiltro, setCategoriaFiltro] = (0, react_1.useState)('');
    const [clienteSeleccionado, setClienteSeleccionado] = (0, react_1.useState)('');
    const [metodoPagoSeleccionado, setMetodoPagoSeleccionado] = (0, react_1.useState)('');
    const [referenciaPago, setReferenciaPago] = (0, react_1.useState)('');
    // Nuevos estados para funcionalidades avanzadas
    const [descuentoGlobal, setDescuentoGlobal] = (0, react_1.useState)({ tipo: 'PORCENTAJE', valor: 0, descripcion: '' });
    const [modoVentaRapida, setModoVentaRapida] = (0, react_1.useState)(false);
    const [ventaSinStock, setVentaSinStock] = (0, react_1.useState)(true);
    const [estadoVentaActual, setEstadoVentaActual] = (0, react_1.useState)({ estado: 'EN_PROGRESO', fecha_estado: new Date().toISOString() });
    const [busquedaPorCodigoBarras, setBusquedaPorCodigoBarras] = (0, react_1.useState)('');
    // Estados de modales
    const [mostrarModalAbrirCaja, setMostrarModalAbrirCaja] = (0, react_1.useState)(false);
    const [mostrarModalCerrarCaja, setMostrarModalCerrarCaja] = (0, react_1.useState)(false);
    const [mostrarModalPago, setMostrarModalPago] = (0, react_1.useState)(false);
    // Estados de formularios
    const [montoInicialInput, setMontoInicialInput] = (0, react_1.useState)('');
    const [montoContadoInput, setMontoContadoInput] = (0, react_1.useState)('');
    const [notasCierreInput, setNotasCierreInput] = (0, react_1.useState)('');
    const [isLoading, setIsLoading] = (0, react_1.useState)(true);
    const [empresaInfo, setEmpresaInfo] = (0, react_1.useState)(null);
    const [detallesFactura, setDetallesFactura] = (0, react_1.useState)([]);
    const [loadingFactura, setLoadingFactura] = (0, react_1.useState)(false);
    (0, react_1.useEffect)(() => {
        cargarDatos();
    }, []);
    const cargarDatos = async () => {
        ...
        try {
            ...
            const [clientesRes, metodosPagoRes, cajaRes, empresaRes] = await Promise.all([
                    ...
                    if (venta.observaciones) {
                        try {
                            ...
                            if (observacionesData.items && Array.isArray(observacionesData.items)) {
                                detalles = observacionesData.items.map((item, index) => ({
                                    id: index + 1,
                                    venta_id: venta.id,
    ...
    const aplicarDescuentoItem = (productoId, descuento) => {
...
🚨 LA VENTA NO SE GUARDÓ - REVISA EL ERROR Y CORRIGE LA BD`;
    ...
    const metodoPagoActual = metodosPago.find((m) => m.id === metodoPagoSeleccionado);
    const clienteActual = clientes.find((c) => c.id === clienteSeleccionado);
    return (<>
      {estadoCaja?.estado === 'CERRADA' ? (<div className="dashboard-container">
          <div className="min-h-screen flex items-center justify-center">
            <div className="stat-card" style={{ maxWidth: '500px', textAlign: 'center' }}>
              <div style={{ marginBottom: '2rem' }}>
                ...
                <button onClick={abrirCaja} className="btn btn-primary" style={{
                width: '100%',
                padding: '1.5rem 2rem',
                fontSize: '1.2rem',
          ...
          {mostrarModalAbrirCaja && (<div style={{
                    position: 'fixed',
                    top: 0,
                    left: 0,
                    right: 0,
              ...
              <div className="stat-card" style={{ maxWidth: '400px', textAlign: 'center' }}>
                ...
                <div style={{ marginBottom: '2rem' }}>
                  ...
                  <input type="number" value={montoInicialInput} onChange={(e) => setMontoInicialInput(e.target.value)} placeholder="0.00" style={{
                    width: '100%',
                    padding: '1rem',
                ...
                <div style={{ display: 'flex', gap: '1rem' }}>
                  <button onClick={() => setMostrarModalAbrirCaja(false)} className="btn btn-secondary" style={{ flex: 1 }}>
                    Cancelar
                  </button>
                  <button onClick={confirmarAbrirCaja} className="btn btn-primary" style={{
                    flex: 1,
                    background: 'var(--gradient-success)',
                }}>
                    Abrir Caja
        ...
        </div>) : (<div className="dashboard-container" style={{ padding: '1rem', maxWidth: '100%' }}>
      ...
      <div className="dashboard-header" style={{ marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem' }}>
          <button onClick={recargarProductos} className="btn" style={{
          ...
          <button onClick={cerrarCaja} className="btn" style={{
                background: 'var(--gradient-danger)',
                color: 'white',
                border: 'none',
            }}>
      ...
      <div style={{ display: 'flex', gap: '1rem' }}>
                  <div style={{ flex: 1 }}>
            <input type="text" placeholder="🔍 Buscar por nombre, código o código de barras..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)} style={{
                width: '100%',
                padding: '1rem',
                border: '2px solid var(--primary-300)',
                borderRadius: 'var(--border-radius)',
            ...
            <select value={categoriaFiltro} onChange={(e) => setCategoriaFiltro(e.target.value)} style={{
                padding: '1rem',
                border: '2px solid var(--primary-300)',
                borderRadius: 'var(--border-radius)',
                minWidth: '180px',
              ...
              <div className="stat-card" style={{ marginBottom: '1rem', padding: '1rem' }}>
                ...
                fontSize: '0.75rem',
                fontWeight: '600',
                backgroundColor: estadoVentaActual.estado === 'EN_PROGRESO'
                    ? 'var(--blue-100)'
                    : estadoVentaActual.estado === 'PENDIENTE_PAGO'
                        ...
                        : estadoVentaActual.estado === 'PAGADA'
                            ? 'var(--emerald-100)'
                            : 'var(--red-100)',
                color: estadoVentaActual.estado === 'EN_PROGRESO'
                    ? 'var(--blue-800)'
                    : estadoVentaActual.estado === 'PENDIENTE_PAGO'
                        ? 'var(--amber-800)'
                        : estadoVentaActual.estado === 'PAGADA'
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <input type="text" placeholder="📱 Código de barras" value={busquedaPorCodigoBarras} onChange={(e) => setBusquedaPorCodigoBarras(e.target.value)} style={{
                padding: '0.5rem',
                ...
                width: '150px',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    ...
                    <select value={descuentoGlobal.tipo} onChange={(e) => setDescuentoGlobal({
                ...descuentoGlobal,
                tipo: e.target.value,
            })} style={{
                padding: '0.5rem',
                ...
                fontSize: '0.875rem',
                    ...
                    <input type="number" value={descuentoGlobal.valor} onChange={(e) => setDescuentoGlobal({
                ...descuentoGlobal,
                ...
                cursor: 'pointer',
                      ...
                      <input type="checkbox" checked={modoVentaRapida} onChange={(e) => setModoVentaRapida(e.target.checked)} style={{ transform: 'scale(1.2)' }}/>
                      ⚡ Rápida
                    </label>
                    <label style={{
                display: 'flex',
                ...
                cursor: 'pointer',
                      ...
                      <input type="checkbox" checked={ventaSinStock} onChange={(e) => setVentaSinStock(e.target.checked)} style={{ transform: 'scale(1.2)' }}/>
                      📦 Sin Stock
                    </label>
                  </div>
                </div>
              ...
              <div style={{
                ...
                gap: '1rem',
                  ...
                  {productosFiltrados.map((producto) => (<div key={producto.id} onClick={() => agregarAlCarrito(producto)} className="stat-card" style={{
                    cursor: 'pointer',
                    padding: '1rem',
                    ...
                    marginBottom: '1rem',
                        ...
                        {producto.imagen_url ? (<img src={producto.imagen_url} alt={producto.nombre} style={{
                        width: '100%',
                        height: '100%',
                        objectFit: 'cover',
                        borderRadius: 'var(--border-radius)',
                    ...
                    borderRadius: '999px',
                    backgroundColor: producto.stock_actual > producto.stock_minimo
                        ? 'var(--emerald-100)'
                        : producto.stock_actual > 0
                            ? 'var(--amber-100)'
                            : 'var(--red-100)',
                    color: producto.stock_actual > producto.stock_minimo
                        ? 'var(--emerald-800)'
                        : producto.stock_actual > 0
                            ? 'var(--amber-800)'
                            : 'var(--red-800)',
            ...
            <div style={{
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '1rem' }}>
                {carrito.length === 0 ? (<div style={{
                  ...
                  </div>) : (carrito.map((item) => (<div key={item.producto.id} className="stat-card" style={{
                    padding: '1rem',
                    marginBottom: '1rem',
                    display: 'flex',
                    gap: '1rem',
                    alignItems: 'center',
                      ...
                      <div style={{ flexShrink: 0 }}>
                        {item.producto.imagen_url ? (<img src={item.producto.imagen_url} alt={item.producto.nombre} style={{
                        width: '60px',
                        height: '60px',
                        objectFit: 'cover',
                        borderRadius: 'var(--border-radius)',
                    ...
                    fontSize: '0.875rem',
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.5rem' }}>
                          <button onClick={() => actualizarCantidad(item.producto.id, item.cantidad - 1)} className="btn-icon">
                            -
                          </button>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        ...
                        <button onClick={() => eliminarDelCarrito(item.producto.id)} className="btn-icon-danger" title="Eliminar producto del carrito">
                          🗑️
                        </button>
                </div>
              </div>)))}
            ...
            <div style={{
              ...
              <div className="stat-card" style={{
                ...
                <div style={{ display: 'flex', gap: '1rem' }}>
                  <select value={clienteSeleccionado} onChange={(e) => setClienteSeleccionado(e.target.value)} style={{
                flex: 1,
                padding: '0.75rem',
                border: '1px solid var(--primary-300)',
                borderRadius: '4px',
                ...
                {clienteActual && (<div style={{ fontSize: '0.8rem', marginTop: '0.5rem', color: 'var(--primary-600)' }}>
              ...
              <div className="stat-card" style={{
                ...
                marginBottom: '1rem',
                  ...
                  {metodosPago.map((metodo) => (<button key={metodo.id} onClick={() => setMetodoPagoSeleccionado(metodo.id)} className={`btn ${metodoPagoSeleccionado === metodo.id ? 'btn-primary' : 'btn-secondary'}`}>
                      {metodo.nombre}
            </button>))}
                </div>
                {metodoPagoActual?.requiere_referencia && (<input type="text" value={referenciaPago} onChange={(e) => setReferenciaPago(e.target.value)} placeholder="N° de referencia / operación" style={{
                    width: '100%',
                    padding: '0.75rem',
                    border: '1px solid var(--primary-300)',
                    borderRadius: '4px',
            ...
            <button onClick={procesarVenta} disabled={carrito.length === 0 || !metodoPagoSeleccionado} className="btn btn-primary" style={{
                padding: '1.5rem',
                fontSize: '1.25rem',
                background: 'var(--gradient-success)',
                color: 'white',
                ...
                <div style={{ display: 'flex', gap: '1rem' }}>
                  <button className="btn btn-secondary" style={{ flex: 1 }} onClick={() => setCarrito([])}>
                    Cancelar
                  </button>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {historialVentas.map((venta) => (<tr key={venta.id} style={{
                    borderBottom: '1px solid var(--primary-200)',
                    background: facturaSeleccionada?.id === venta.id ? 'var(--blue-100)' : 'transparent',
                }}>
                    ...
                    borderRadius: '999px',
                    fontSize: '0.75rem',
                    backgroundColor: venta.estado === 'PAGADA' ? 'var(--emerald-100)' : 'var(--amber-100)',
      ...
      {facturaSeleccionada && (<div style={{
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <button onClick={() => setFacturaSeleccionada(null)} style={{
                ...
                <div style={{ overflowY: 'auto', padding: '2rem', fontFamily: 'sans-serif', color: '#374151' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '1.5rem' }}>
                        <div>
                            {empresaInfo?.logo_url ? (<img src={empresaInfo.logo_url} alt="Logo de la empresa" style={{ maxHeight: '60px', marginBottom: '1rem' }}/>) : (<h1 style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#111827' }}>
                    ...
                    {/* Tabla de Items */}
                    {loadingFactura ? (<p style={{ textAlign: 'center', padding: '2rem' }}>Cargando detalles...</p>) : (<table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                            ...
                            <tbody>
                                {detallesFactura.map(item => (<tr key={item.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
