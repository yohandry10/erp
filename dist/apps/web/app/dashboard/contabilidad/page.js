...
exports.default = ContabilidadPage;
const react_1 = require("react");
const use_api_1 = require("@/hooks/use-api");
function ContabilidadPage() {
    const [estadoResultados, setEstadoResultados] = (0, react_1.useState)(null);
    const [balanceGeneral, setBalanceGeneral] = (0, react_1.useState)(null);
    const [libroDiario, setLibroDiario] = (0, react_1.useState)(null);
    const [libroMayor, setLibroMayor] = (0, react_1.useState)(null);
    const [registroVentas, setRegistroVentas] = (0, react_1.useState)(null);
    const [periodo, setPeriodo] = (0, react_1.useState)('mes-actual');
    const [vistaActual, setVistaActual] = (0, react_1.useState)('estado-resultados');
        ...
        try {
            ...
            switch (tipo) {
                case 'libro-diario':
                    ...
                    datosExcel = libroDiario.asientos.flatMap(asiento => asiento.detalles.map(detalle => ({
    ...
    const renderEstadoResultados = () => {
        if (!estadoResultados || !estadoResultados.ingresos) {
            return (<div className="activity-card" style={{ textAlign: 'center', padding: '2rem' }}>
    ...
    const renderLibroDiario = () => {
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <h2>📖 Libro Diario</h2>
          <button onClick={() => exportarExcel('libro-diario')} className="export-button" style={{
                padding: '0.5rem 1rem',
                backgroundColor: '#10b981',
                color: 'white',
                border: 'none',
        ...
        <div style={{ overflow: 'auto' }}>
          {libroDiario.asientos.map((asiento) => (<div key={asiento.numeroAsiento} style={{ marginBottom: '2rem', border: '1px solid #e5e7eb', borderRadius: '8px' }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {asiento.detalles.map((detalle, idx) => (<tr key={idx} style={{ borderBottom: '1px solid #f3f4f6' }}>
    ...
    const renderLibroMayor = () => {
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <h2>📊 Libro Mayor</h2>
          <button onClick={() => exportarExcel('libro-mayor')} style={{
                padding: '0.5rem 1rem',
                backgroundColor: '#10b981',
                color: 'white',
                border: 'none',
        ...
        <div style={{ overflow: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {libroMayor.cuentas.map((cuenta) => (<tr key={cuenta.codigo} style={{ borderBottom: '1px solid #e5e7eb' }}>
    ...
    const renderRegistroVentas = () => {
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          ...
          <button onClick={() => exportarExcel('registro-ventas')} style={{
                padding: '0.5rem 1rem',
                backgroundColor: '#10b981',
                color: 'white',
                border: 'none',
        ...
        <div style={{ overflow: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {registroVentas.ventas.map((venta, idx) => (<tr key={idx} style={{ borderBottom: '1px solid #e5e7eb' }}>
    ...
    return (<div className="dashboard-container">
      <div className="dashboard-header">
        ...
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {(periodo === 'personalizado' || ['libro-diario', 'libro-mayor', 'registro-ventas'].includes(vistaActual)) && (<>
              <input type="date" value={filtros.fechaDesde} onChange={(e) => setFiltros(prev => ({ ...prev, fechaDesde: e.target.value }))} style={{
                padding: '0.5rem',
                borderRadius: '6px',
                border: '1px solid rgba(0,0,0,0.1)',
                fontSize: '0.875rem'
            }} placeholder="Fecha desde"/>
              <input type="date" value={filtros.fechaHasta} onChange={(e) => setFiltros(prev => ({ ...prev, fechaHasta: e.target.value }))} style={{
                padding: '0.5rem',
                borderRadius: '6px',
                border: '1px solid rgba(0,0,0,0.1)',
                fontSize: '0.875rem'
            }} placeholder="Fecha hasta"/>
              <button onClick={aplicarFiltros} disabled={loading} style={{
                padding: '0.5rem 1rem',
                backgroundColor: '#3b82f6',
                color: 'white',
                border: 'none',
      ...
      <div style={{ marginBottom: '2rem' }}>
        <div style={{ display: 'flex', gap: '0.5rem', backgroundColor: '#f3f4f6', padding: '0.5rem', borderRadius: '8px', flexWrap: 'wrap' }}>
          <button onClick={() => setVistaActual('estado-resultados')} style={{
          ...
          <button onClick={() => setVistaActual('balance-general')} style={{
            padding: '0.75rem 1.5rem',
          ...
          <button onClick={() => setVistaActual('libro-diario')} style={{
            padding: '0.75rem 1.5rem',
          ...
          <button onClick={() => setVistaActual('libro-mayor')} style={{
            padding: '0.75rem 1.5rem',
          ...
          <button onClick={() => setVistaActual('registro-ventas')} style={{
            padding: '0.75rem 1.5rem',
