...
exports.default = AsistenciaComponent;
...
function AsistenciaComponent() {
    ...
    const [asistenciaHoy, setAsistenciaHoy] = (0, react_1.useState)([]);
    const [loading, setLoading] = (0, react_1.useState)(false);
    const [fechaConsulta, setFechaConsulta] = (0, react_1.useState)(new Date().toISOString().split('T')[0]);
    const [modoMarcado, setModoMarcado] = (0, react_1.useState)('individual');
    (0, react_1.useEffect)(() => {
        loadEmpleados();
    ...
    const loadEmpleados = async () => {
        try {
            ...
            if (response?.success) {
                const empleadosActivos = response.data.filter(emp => emp.estado === 'activo');
    ...
    const marcarTodosPresentes = async () => {
        try {
            setLoading(true);
            const empleadosSinMarcar = empleados.filter(emp => !asistenciaHoy.find(asist => asist.id_empleado === emp.id));
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right">
              ...
              <input type="date" value={fechaConsulta} onChange={(e) => setFechaConsulta(e.target.value)} className="p-2 border border-blue-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"/>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex gap-3">
            <button onClick={() => setModoMarcado('individual')} className={`px-4 py-2 rounded-lg transition-colors ${modoMarcado === 'individual'
            ? 'bg-blue-100 text-blue-700 border border-blue-300'
            : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
              👤 Individual
            </button>
            <button onClick={() => setModoMarcado('masivo')} className={`px-4 py-2 rounded-lg transition-colors ${modoMarcado === 'masivo'
            ? 'bg-blue-100 text-blue-700 border border-blue-300'
            : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
              👥 Masivo
            </button>
        ...
        {modoMarcado === 'masivo' && (<div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-4">
            ...
            <button onClick={marcarTodosPresentes} disabled={loading} className="bg-amber-500 text-white px-4 py-2 rounded-lg hover:bg-amber-600 disabled:bg-gray-400 transition-colors">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="divide-y divide-gray-100">
          {empleados.map((empleado) => {
            ...
            return (<div key={empleado.id} className="p-4 hover:bg-gray-50 transition-colors">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    {/* Avatar */}
                    <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center">
                      {empleado.foto_url ? (<img src={empleado.foto_url} alt={empleado.nombres} className="w-12 h-12 rounded-full object-cover"/>) : (<span className="text-blue-600 font-semibold text-lg">
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div className="text-right text-sm">
                      {registro?.hora_entrada && (<div className="text-green-600 font-medium">
                          📥 Entrada: {formatearHora(registro.hora_entrada)}
                          {registro.tardanza_minutos > 0 && (<span className="text-amber-600 ml-2">
                              (+{registro.tardanza_minutos}min tarde)
                            </span>)}
                        </div>)}
                      {registro?.hora_salida && (<div className="text-blue-600 font-medium">
                          📤 Salida: {formatearHora(registro.hora_salida)}
                        </div>)}
                      {registro?.hora_entrada && registro?.hora_salida && (<div className="text-purple-600 font-medium">
                    ...
                    <div className="flex gap-2">
                      {fechaConsulta === new Date().toISOString().split('T')[0] && (<>
                          {!registro?.hora_entrada && (<button onClick={() => marcarAsistencia(empleado.id, 'entrada')} disabled={loading} className="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 disabled:bg-gray-400 transition-colors text-sm font-medium">
                              📥 Marcar Entrada
                            </button>)}
                          
                          {registro?.hora_entrada && !registro?.hora_salida && (<button onClick={() => marcarAsistencia(empleado.id, 'salida')} disabled={loading} className="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 disabled:bg-gray-400 transition-colors text-sm font-medium">
                              📤 Marcar Salida
                            </button>)}

                          {registro?.hora_entrada && registro?.hora_salida && (<div className="bg-gray-100 text-gray-600 px-4 py-2 rounded-lg text-sm font-medium">
                              ✅ Jornada completa
                            </div>)}
                        </>)}
                      
                      {fechaConsulta !== new Date().toISOString().split('T')[0] && (<div className="text-gray-500 text-sm italic">
                          📅 Consulta histórica
                        </div>)}
                    </div>
                  </div>
        ...
        {empleados.length === 0 && (<div className="p-8 text-center text-gray-500">
