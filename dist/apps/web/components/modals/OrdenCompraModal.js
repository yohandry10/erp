...
exports.default = OrdenCompraModal;
const react_1 = require("react");
const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001';
function OrdenCompraModal({ isOpen, onClose, onSuccess, orden }) {
    ...
    const [proveedores, setProveedores] = (0, react_1.useState)([]);
    const [productos, setProductos] = (0, react_1.useState)([]);
    const [isLoading, setIsLoading] = (0, react_1.useState)(false);
    const [totales, setTotales] = (0, react_1.useState)({
        subtotal: 0,
        igv: 0,
        total: 0
    });
    ...
    const loadOrdenData = () => {
        if (orden) {
            ...
            const itemsToLoad = itemsArray.map((item, index) => {
                const processedItem = {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
            <div>
              ...
              <input type="text" value={formData.numero} disabled style={{
            width: '100%',
            padding: '0.5rem',
            ...
            <div>
              ...
              <select value={formData.proveedor_id} onChange={(e) => setFormData({ ...formData, proveedor_id: e.target.value })} required style={{
            width: '100%',
            padding: '0.5rem',
            border: '1px solid #d1d5db',
            borderRadius: '0.375rem',
            backgroundColor: 'white'
                ...
                {proveedores.map((proveedor) => (<option key={proveedor.id} value={proveedor.id}>
                    {proveedor.nombre}
                  </option>))}
              </select>
            </div>
            ...
            <div>
              ...
              <select value={formData.moneda} onChange={(e) => setFormData({ ...formData, moneda: e.target.value })} style={{
            width: '100%',
            padding: '0.5rem',
            border: '1px solid #d1d5db',
            borderRadius: '0.375rem',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
            <div>
              ...
              <input type="date" value={formData.fecha_orden} onChange={(e) => setFormData({ ...formData, fecha_orden: e.target.value })} required style={{
            width: '100%',
            padding: '0.5rem',
            border: '1px solid #d1d5db',
            borderRadius: '0.375rem'
            ...
            <div>
              ...
              <input type="date" value={formData.fecha_entrega} onChange={(e) => setFormData({ ...formData, fecha_entrega: e.target.value })} required style={{
            width: '100%',
            padding: '0.5rem',
            border: '1px solid #d1d5db',
            borderRadius: '0.375rem'
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            <div style={{ border: '1px solid #e5e7eb', borderRadius: '0.5rem', overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {items.map((item, index) => (<tr key={item.id} style={{ borderTop: '1px solid #e5e7eb' }}>
                      <td style={{ padding: '0.75rem' }}>
                        {item.esNuevoProducto ? (<div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                            <input type="text" placeholder="Nombre del nuevo producto" value={item.producto_nombre || ''} onChange={(e) => updateItem(index, 'producto_nombre', e.target.value)} required style={{
                    flex: 1,
                    ...
                    backgroundColor: '#eff6ff'
                            ...
                            <button type="button" onClick={() => updateItem(index, 'esNuevoProducto', false)} style={{
                    ...
                    fontSize: '0.75rem'
                            ...
                            <select value={item.producto_id} onChange={(e) => updateItem(index, 'producto_id', e.target.value)} required style={{
                    flex: 1,
                    padding: '0.25rem 0.5rem',
                    border: '1px solid #d1d5db',
                    borderRadius: '0.25rem',
                    fontSize: '0.875rem'
                            ...
                            <button type="button" onClick={() => updateItem(index, 'esNuevoProducto', true)} style={{
                    ...
                    fontSize: '0.75rem'
                      ...
                      <td style={{ padding: '0.75rem', textAlign: 'center' }}>
                        <input type="number" min="1" value={item.cantidad || ''} onChange={(e) => updateItem(index, 'cantidad', e.target.value)} style={{
                width: '80px',
                padding: '0.25rem',
                border: '1px solid #d1d5db',
                borderRadius: '0.25rem',
                ...
                fontSize: '0.875rem'
                      ...
                      <td style={{ padding: '0.75rem', textAlign: 'right' }}>
                        <input type="number" step="0.01" min="0" value={item.precio_unitario || ''} onChange={(e) => updateItem(index, 'precio_unitario', e.target.value)} style={{
                width: '100px',
                padding: '0.25rem',
                border: '1px solid #d1d5db',
                borderRadius: '0.25rem',
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            <textarea value={formData.observaciones} onChange={(e) => setFormData({ ...formData, observaciones: e.target.value })} rows={3} placeholder="Observaciones adicionales..." style={{
            width: '100%',
            padding: '0.5rem',
            border: '1px solid #d1d5db',
            borderRadius: '0.375rem',
