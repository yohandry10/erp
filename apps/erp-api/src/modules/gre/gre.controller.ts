...
export class GreController {
  ...
  async reenviarGre(
    ...
    try {
      const resultado = await this.greService.reenviarGre(id, tenantId, { idempotencyKey });
...
<DespatchAdvice>
  ...
  async enviarManualmenteSunat(
    ...
    try {
      ...
      const resultado = await this.greService.enviarManualmenteSunat(id, tenantId, { idempotencyKey });
