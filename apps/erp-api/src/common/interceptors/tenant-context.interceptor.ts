...
export class TenantContextInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    // ✅ HARDENING: validar/permitir override de tenant SOLO después de JWT verificado
    if (headerTenant && tenantId && headerTenant !== tenantId && !isSuperAdmin) {
      this.logger.warn(
        `Header X-Tenant-Id (${headerTenant}) rechazado: token pertenece a ${tenantId} y no es superadmin`,
      );
      throw new ForbiddenException('Acceso denegado: tenant header inválido');
    }

    // ✅ Superadmin: permitir operar sobre otro tenant vía header (auditar en logs/DB)
    if (headerTenant && isSuperAdmin) {
