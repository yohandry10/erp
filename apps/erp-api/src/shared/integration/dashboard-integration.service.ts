...
export class DashboardIntegrationService {
  ...
  async getRecentActivities(tenantId?: string): Promise<ActivityItem[]> {
    try {
      ...
      const [
        ...
        this.getVentasRecientes(client, hace24h, currentTenantId),
        this.getComprasRecientes(client, hace24h, currentTenantId),
        this.getCotizacionesRecientes(client, hace24h, currentTenantId),
        this.getCpeRecientes(client, hace24h, currentTenantId),
        this.getGreRecientes(client, hace24h, currentTenantId)
      ]);
  ...
  private async getVentasRecientes(client: any, desde: Date, tenantId: string) {
    const { data } = await client.from('ventas_pos')
  ...
  private async getComprasRecientes(client: any, desde: Date, tenantId: string) {
    const { data } = await client.from('ordenes_compra')
  ...
  private async getCotizacionesRecientes(client: any, desde: Date, tenantId: string) {
    const { data } = await client.from('cotizaciones')
  ...
  private async getCpeRecientes(client: any, desde: Date, tenantId: string) {
    const { data } = await client.from('cpe')
  ...
  private async getGreRecientes(client: any, desde: Date, tenantId: string) {
    const { data } = await client.from('gre')
  ...
  private calcularValorInventario(productos: any[]): number {
    return productos?.reduce((sum, p) => 
      sum +
        (parseFloat(p.precio) || 0) * (parseFloat((p as any).stock_actual) || 0),
      0,
    ) || 0;
  }
