...
test('E2E Ventas – Constraint de stock previene valores negativos', async () => {
  ...
  // Si hay constraint CHECK, debe fallar
  // Si no hay constraint, el test documenta que falta
  if (updateError) {
    assert.ok(
      updateError.message.includes('check') || 
      updateError.message.includes('constraint') ||
      updateError.message.includes('violates'),
      'Error debe ser por constraint de stock'
    );
    console.log('✅ Constraint de stock >= 0 está activo');
  } else {
    ...
    if (producto && parseFloat(producto.stock) < 0) {
      console.warn('⚠️ HALLAZGO: No hay constraint CHECK (stock >= 0) en tabla productos');
      console.warn('   Recomendación: Agregar constraint en migración');
    }
  }
...
test('E2E Ventas – Índices existen para consultas frecuentes', async () => {
  ...
  // Consultar índices de la tabla pedidos_venta
  const { data: indices, error } = await supabase.rpc('get_table_indexes', {
    p_table_name: 'pedidos_venta',
  }).catch(() => ({ data: null, error: { message: 'RPC no existe' } }));

  if (error) {
    // Si el RPC no existe, consultar directamente pg_indexes
    const { data: pgIndices } = await supabase
      .from('pg_indexes')
      .select('indexname, indexdef')
      .eq('tablename', 'pedidos_venta')
      .catch(() => ({ data: null }));

    if (pgIndices) {
      console.log('Índices encontrados en pedidos_venta:', pgIndices.map(i => i.indexname));
    } else {
      console.log('⚠️ No se pudo verificar índices (requiere acceso a pg_indexes)');
    }
    return; // Test informativo
  }
