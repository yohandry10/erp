...
let SireService = class SireService {
    ...
    async procesarComprobanteParaSire(comprobante) {
        try {
    ...
    async buscarOCrearReportePeriodo(periodo, tipoDocumento) {
        try {
    ...
    async actualizarContadorRegistros(reporteId) {
        try {
    ...
    async actualizarRegistrosPendientes(periodo, tipoDocumento) {
        console.log(`📊 [SIRE] Registro pendiente: ${tipoDocumento} para período ${periodo}`);
    }
    async crearRegistroDetalleComprobante(reporteId, comprobante) {
        try {
    ...
    async getReportes(filters, tenantId) {
        try {
            ...
            if (error) {
                ...
                throw new common_1.BadRequestException('Error fetching SIRE reports: ' + error.message);
            }
    ...
    getTipoReporteFullName(tipoCorto) {
        const tipoMap = {
    ...
    async generarReporte(reportData, tenantId) {
        try {
            console.log('🔄 Generando reporte SIRE:', reportData, 'para tenant:', tenantId);
            if (!reportData.tipoReporte || !reportData.periodo) {
                throw new common_1.BadRequestException('Tipo de reporte y período son requeridos');
            }
            const tipoReporteMap = {
                'REGISTRO_VENTAS': 'REG_VEN',
                'REGISTRO_COMPRAS': 'REG_COM',
            ...
            const periodoCorto = reportData.periodo.substring(0, 10);
            const nuevoReporte = {
            ...
            if (error) {
                ...
                throw new common_1.BadRequestException('Error creating SIRE report: ' + error.message);
            }
            ...
            if (!reporteCreado) {
                ...
                if (queryError || !ultimoReporte) {
                    ...
                    throw new common_1.BadRequestException('Error verificando reporte creado');
                }
            ...
            throw new common_1.BadRequestException('Error al generar el reporte SIRE');
        }
    }
    async downloadReporte(id, tenantId) {
        try {
            ...
            if (error || !reporte) {
                throw new common_1.NotFoundException('Reporte SIRE no encontrado');
            }
            if (reporte.estado !== 'GENERADO' && reporte.estado !== 'ENVIADO') {
                throw new common_1.BadRequestException('El reporte aún no está disponible para descarga');
            }
            ...
            throw new common_1.BadRequestException('Error al descargar el reporte');
        }
    }
    async enviarSunat(id, tenantId) {
        try {
            ...
            if (reporte.estado !== 'GENERADO') {
                throw new common_1.BadRequestException('El reporte debe estar generado para poder enviarse');
            }
            const { error: updateError } = await this.supabaseService.update('sire_files', {
                estado: 'ENVIADO',
                updated_at: new Date().toISOString(),
            }, { id });
            if (updateError) {
                throw new common_1.BadRequestException('Error updating report status: ' + updateError.message);
            }
            ...
            throw new common_1.BadRequestException('Error al enviar el reporte a SUNAT');
        }
    }
    findAll() {
        return {
    ...
    async simularGeneracionReporte(reporteId) {
        try {
    ...
    generarRegistroCompras(reporte) {
...
exports.SireService = SireService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [supabase_service_1.SupabaseService,
        event_bus_service_1.EventBusService])
], SireService);
//# sourceMappingURL=sire.service.js.map