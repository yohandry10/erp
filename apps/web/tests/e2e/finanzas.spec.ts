...
test.describe("Finanzas - Tesorería", () => {
  ...
  test("Aplicar pago a CxP", async ({ page }) => {
    ...
    await expect(
      cxpRows.first(),
      "Debe renderizarse al menos una CxP para validar el pago real",
    ).toBeVisible({ timeout: 30000 });
    const rowCount = await cxpRows.count();
    expect(
      rowCount,
      "Debe existir al menos una CxP para validar el pago real",
    ).toBeGreaterThan(0);

    // Open the first CxP through the explicit row action.
    const firstRow = cxpRows.first();
    const verButton = firstRow
      .locator('a:has-text("Ver"), button:has-text("Ver")')
      .first();
    ...
    await page.waitForURL(
      (url) =>
        url.href !== listUrl &&
        /\/dashboard\/finanzas\/cxp\/[^/]+\/?$/.test(url.pathname),
      { timeout: 10000 },
    );
    ...
    await page.screenshot({
      path: "tests/screenshots/cxp-detail-before-payment.png",
      fullPage: true,
    });
    ...
    const pagoModal = page.getByRole("dialog", {
      name: "Aplicar Pago a Cuenta por Pagar",
    });
    await expect(pagoModal).toBeVisible({ timeout: 5000 });

    // Take screenshot of the payment modal
    await page.screenshot({
      path: "tests/screenshots/cxp-payment-modal.png",
      fullPage: true,
    });
    ...
    await pagoModal
      .getByPlaceholder("Ej: OP-2025-001234, Cheque #12345")
      .fill(referenciaPago);

    // Fill in observations
    await pagoModal
      .getByPlaceholder("Observaciones adicionales sobre el pago...")
      .fill("Pago de prueba E2E automatizado");

    // Take screenshot with payment details filled
    await page.screenshot({
      path: "tests/screenshots/cxp-payment-modal-filled.png",
      fullPage: true,
    });
    ...
    const historialSection = page.getByRole("heading", {
      name: "Historial de Pagos",
    });
    await expect(historialSection).toBeVisible({ timeout: 10000 });
    await expect(page.getByText(referenciaPago)).toBeVisible({
      timeout: 10000,
    });

    // Take screenshot of the updated CxP
    await page.screenshot({
      path: "tests/screenshots/cxp-after-payment.png",
      fullPage: true,
    });
  ...
  test("Pago masivo de proveedores", async ({ page }) => {
    ...
    await page.screenshot({
      path: "tests/screenshots/tesoreria-dashboard.png",
      fullPage: true,
    });

    // Look for the "Pago Masivo" button or link
    const pagoMasivoButton = page.locator(
      'a[href*="/tesoreria/lote"], button:has-text("Pago Masivo")',
    );
    ...
    await expect(
      page.getByText("No hay cuentas bancarias disponibles"),
    ).toHaveCount(0);
    await expect(
      page.getByText("No hay cuentas por pagar pendientes"),
    ).toHaveCount(0);

    // Take screenshot of pago lote page
    await page.screenshot({
      path: "tests/screenshots/pago-lote-initial.png",
      fullPage: true,
    });
    ...
    await expect(
      ...
      "Debe quedar seleccionada una cuenta bancaria real",
    ).not.toContainText("Seleccione una cuenta");
    ...
    const referenciaInput = page.locator(
      '#referencia-lote, input[placeholder*="LOTE"]',
    );
    await expect(
      referenciaInput.first(),
      "El flujo de pago masivo debe permitir referencia única de lote",
    ).toBeVisible({ timeout: 10000 });
    await referenciaInput.first().fill(referenciaLote);

    // Take screenshot of step 1 completed
    await page.screenshot({
      path: "tests/screenshots/pago-lote-step1-filled.png",
      fullPage: true,
    });
    ...
    await expect(
      page.getByText("No hay cuentas por pagar que coincidan con los filtros"),
    ).toHaveCount(0);
    ...
    await page.screenshot({
      path: "tests/screenshots/pago-lote-step2-selected.png",
      fullPage: true,
    });
    ...
    await expect(
      page.locator("text=/Saldo (Actual|Después|Insuficiente)/").first(),
    ).toBeVisible();

    // Take screenshot of confirmation step
    await page.screenshot({
      path: "tests/screenshots/pago-lote-step3-confirmation.png",
      fullPage: true,
    });
    ...
    const procesarLoteButton = page.locator(
      'button:has-text("Procesar Lote"), button:has-text("Confirmar")',
    );
    ...
    await expect(page.getByText("Lote Procesado Exitosamente")).toBeVisible({
      timeout: 10000,
    });
    ...
    await page.screenshot({
      path: "tests/screenshots/pago-lote-result.png",
      fullPage: true,
    });

    // Verify batch details are displayed
    await expect(
      page.getByText("Referencia del Lote", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByText("Pagos Exitosos", { exact: true }),
    ).toBeVisible();
    await expect(page.getByText("Monto Total", { exact: true })).toBeVisible();
    await expect(
      page.getByText("Detalle de Pagos", { exact: true }),
    ).toBeVisible();

    // Verify action buttons are available
    const volverButton = page.locator(
      'button:has-text("Volver"), a:has-text("Volver")',
    );
    await expect(
      volverButton.first(),
      "El resultado de lote debe permitir volver a Tesorería",
    ).toBeVisible({ timeout: 10000 });

    console.log(
      "✅ Test de pago masivo de proveedores completado exitosamente",
    );
  });
  ...
  test("Importar extracto y conciliar", async ({ page }) => {
    ...
    await page.screenshot({
      path: "tests/screenshots/conciliacion-list.png",
      fullPage: true,
    });
    ...
    if (rowCount === 0) {
      ...
      const nuevaConciliacionButton = page.locator(
        'button:has-text("Nueva Conciliación"), a:has-text("Nueva Conciliación")',
      );

      await expect(
        nuevaConciliacionButton.first(),
        "Debe existir acción para crear conciliación",
      ).toBeVisible();
      ...
      const procesarButton = page
        .getByRole("button", { name: /Procesar|Ver/ })
        .first();
      await expect(
        procesarButton,
        "Debe poder abrirse la conciliación creada",
      ).toBeVisible({ timeout: 15000 });
      await procesarButton.click();
      await page.waitForURL(/\/dashboard\/finanzas\/conciliacion\/[^/]+\/?$/, {
        timeout: 10000,
      });
    } else {
      // Click on first conciliación in ABIERTA state
      const firstRowAction = conciliacionRows
        .first()
        .getByRole("button", { name: /Procesar|Ver/ })
        .first();
      await expect(
        firstRowAction,
        "La primera conciliación debe tener acción para abrir el detalle",
      ).toBeVisible();
    ...
    await page.screenshot({
      path: "tests/screenshots/conciliacion-detail-initial.png",
      fullPage: true,
    });

    // STEP 1: Import CSV
    const importarButton = page.locator(
      'button:has-text("Importar Extracto CSV")',
    );
    ...
    await page
      ...
      .catch(() => undefined);
    await expect(
      page.getByRole("heading", { name: "Importar Extracto Bancario" }),
    ).toBeVisible({ timeout: 5000 });

    // Take screenshot of import modal
    await page.screenshot({
      path: "tests/screenshots/conciliacion-import-modal.png",
      fullPage: true,
    });
...
${todayCsv},COMISION BANCARIA,EXT-E2E-COMISION-${csvRunId},CARGO,25.00`;
    ...
    await expect(
      fileInput,
      "El modal de importación debe exponer un input[type=file]",
    ).toHaveCount(1);

    {
      ...
      await page.screenshot({
        path: "tests/screenshots/conciliacion-csv-uploaded.png",
        fullPage: true,
      });
      ...
      await expect(
        previewTable,
        "La importación CSV debe mostrar previsualización tabular antes de confirmar",
      ).toBeVisible({ timeout: 10000 });

      // Click "Importar" button
      const importarConfirmButton = page
        .locator('button:has-text("Importar"), button:has-text("Confirmar")')
        .last();
    ...
    await page.screenshot({
      path: "tests/screenshots/conciliacion-after-import.png",
      fullPage: true,
    });

    // STEP 2: Match Automático
    const matchAutomaticoButton = page.locator(
      'button:has-text("Match Automático")',
    );
    await expect(
      matchAutomaticoButton,
      "La conciliación debe exponer acción de match automático",
    ).toBeVisible({ timeout: 10000 });
    ...
    await page
      ...
      .catch(() => undefined);
    await page.screenshot({
      path: "tests/screenshots/conciliacion-after-auto-match.png",
      fullPage: true,
    });
    ...
    await expect(
      page.getByRole("heading", {
        name: "Movimientos Pendientes de Conciliar",
      }),
    ).toBeVisible({ timeout: 15000 });
    await expect(
      page.getByText("MOVIMIENTOS DEL SISTEMA", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByText("MOVIMIENTOS DEL EXTRACTO", { exact: true }),
    ).toBeVisible();

    // Take screenshot of dual table
    await page.screenshot({
      path: "tests/screenshots/conciliacion-dual-table.png",
      fullPage: true,
    });
    ...
    await expect(
      sistemaItems.first(),
      "El match manual debe cargar movimientos de sistema pendientes",
    ).toBeVisible({ timeout: 15000 });
    await expect(
      extractoItems.first(),
      "El match manual debe cargar movimientos de extracto pendientes",
    ).toBeVisible({ timeout: 15000 });
    ...
    if ((await sistemaCargo.count()) > 0 && (await extractoCargo.count()) > 0) {
      ...
      (await sistemaAbono.count()) > 0 &&
      (await extractoAbono.count()) > 0
    ) {
      await sistemaAbono.click();
      await extractoAbono.click();
    } else {
      throw new Error(
        "Debe existir al menos un par de movimientos del mismo tipo para validar match manual",
      );
    }

    await expect(matchDialog.getByText("Resumen del Match")).toBeVisible({
      timeout: 5000,
    });

    const confirmarMatchButton = matchDialog.getByRole("button", {
      name: "Realizar Match",
    });
    await expect(
      confirmarMatchButton,
      "El match manual debe habilitar confirmación tras seleccionar pares válidos",
    ...
    if (await matchDialog.isVisible({ timeout: 1000 }).catch(() => false)) {
      await matchDialog
        .getByRole("button", { name: /Cancelar|Cerrar/ })
        .click();
    }

    // STEP 5: Review differences
    const cerrarConciliacionButton = page.locator(
      'button:has-text("Cerrar Conciliación")',
    );
    await expect(
      cerrarConciliacionButton,
      "La conciliación debe exponer revisión/cierre",
    ).toBeVisible({ timeout: 10000 });
    ...
    await page.screenshot({
      path: "tests/screenshots/conciliacion-close-confirmation.png",
      fullPage: true,
    });

    // Verify summary information is displayed
    await expect(
      page.getByText("Resumen de Conciliación", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Saldo Libro" }).first(),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Saldo Banco" }).first(),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Diferencia" }).first(),
    ).toBeVisible();

    // Verify movimientos statistics
    await expect(
      page.getByRole("heading", { name: "Movimientos del Sistema" }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Movimientos del Extracto" }),
    ).toBeVisible();

    // Check if there are pending items
    const pendingWarning = page.getByRole("heading", {
      name: /Advertencia: Movimientos Pendientes/,
    });
    const hasPendingItems = (await pendingWarning.count()) > 0;

    if (hasPendingItems) {
      console.log(
        "⚠️ Hay movimientos pendientes. No se puede cerrar sin forzar.",
      );
      ...
      const forzarCierreButton = page.locator(
        'button:has-text("Forzar Cierre")',
      );

      await expect(
        forzarCierreButton,
        "Si hay pendientes, debe existir opción explícita de forzar cierre",
      ).toBeVisible({ timeout: 5000 });
      ...
      const confirmarCierreButton = page
        .locator('button:has-text("Cerrar Conciliación")')
        .last();

      await expect(
        confirmarCierreButton,
        "Sin pendientes, debe poder confirmarse el cierre",
      ).toBeEnabled({ timeout: 5000 });
    ...
    await page.screenshot({
      path: "tests/screenshots/conciliacion-final-state.png",
      fullPage: true,
    });
    console.log(
      "✅ Test de importar extracto y conciliar completado exitosamente",
    );
  });
});
