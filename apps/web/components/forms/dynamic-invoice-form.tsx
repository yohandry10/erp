...
const DynamicInvoiceForm: React.FC = () => {
  ...
  const [formData, setFormData] = useState<InvoiceData>({
    ...
    documentoReceptor: '',
    razonSocialReceptor: '',
    moneda: selectedCountry?.codigo_iso === 'PE' ? 'PEN' : 'COP',
    totalGravadas: 0,
    totalImpuestos: 0,
    importeTotal: 0
  });
  ...
  const calculateTotals = () => {
    const impuestos = formData.totalGravadas * (selectedCountry?.codigo_iso === 'PE' ? 0.18 : 0.19);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Factura ${formData.serie}-${formData.numero} creada exitosamente para ${selectedCountry?.nombre}`,
      });
    } catch (error) {
      toast({
        title: "Error",
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <CardTitle>
          {getLabel('nueva_factura', 'Nueva Factura')} - {selectedCountry?.nombre}
        </CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <DynamicField
              ...
              placeholder={selectedCountry?.codigo_iso === 'PE' ? 'RUC/DNI' : 'NIT/CC'}
            />
            <DynamicField
