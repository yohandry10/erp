...
export function PageShell({
  ...
  return (
    <main
      className={cn(
        "mx-auto w-full max-w-[1600px] space-y-5 p-4 text-slate-100 group-data-[erp-theme=light]/dashboard:text-slate-950 md:p-6",
        className,
      )}
    >
      <header className="rounded-3xl border border-cyan-400/20 bg-slate-950/80 p-5 shadow-2xl shadow-blue-950/30 group-data-[erp-theme=light]/dashboard:border-slate-200 group-data-[erp-theme=light]/dashboard:bg-white/90 group-data-[erp-theme=light]/dashboard:shadow-slate-200/70">
        <div className="flex flex-col gap-4">
          <div className="min-w-0">
            ...
            {description ? (
              <p className="mt-2 max-w-3xl text-sm text-slate-300 group-data-[erp-theme=light]/dashboard:text-slate-600 md:text-base">
                {description}
              </p>
            ) : null}
          </div>
