...
const amountClass = 'text-sm font-bold text-primary';
...
export default function ConciliacionTable({
  ...
  const getRowClass = (
    ...
    if (isSelected) {
      return isSistema
        ? 'border-l-4 border-l-blue-600 bg-primary/10'
        : 'border-l-4 border-l-cyan-600 bg-primary/10';
    }
    if (movimiento.conciliado && highlightMatches) {
      if (movimiento.match_automatico) {
        return cn(
          'border-l-4 border-l-cyan-500 bg-primary/10/80 shadow-[inset_0_0_0_1px_rgba(14,116,144,0.12)]',
          isMatchedPairHovered && 'scale-[1.01] bg-primary/10 shadow-[inset_0_0_0_2px_rgba(14,116,144,0.22)]'
        );
      }
      return 'border-l-4 border-l-cyan-500 bg-primary/10/60';
    }
    if (isHovered) {
      return 'bg-muted/30';
    }
  ...
  const renderMovimientoRow = (
    ...
    return (
      <tr
        ...
        className={cn(
          'border-b border-border transition duration-200',
          canBeDragged ? 'cursor-grab' : readOnly ? 'cursor-default' : 'cursor-pointer',
          rowClass,
          isDragging && 'opacity-50',
          isDropTarget && 'scale-[1.01] border-l-4 border-l-blue-500 bg-primary/10 shadow-[inset_0_0_0_2px_rgba(37,99,235,0.22)]'
        )}
      >
        {/* Status Icon */}
            ...
            {movimiento.conciliado ? (
              movimiento.match_automatico ? (
                <div className="relative inline-block">
                  ...
                  <Link2
                    size={10}
                    className="absolute -bottom-0.5 -right-0.5 rounded-full bg-card text-primary"
                  />
                </div>
              ) : (
                <CheckCircle size={18} className={statusIconClass} />
    ...
    <div className="flex flex-col gap-6">
      ...
      {highlightMatches && (
        <div className="flex flex-col gap-4">
          {/* Legend */}
          <div className="flex flex-wrap gap-6 rounded-lg border border-border bg-muted/30 p-4 text-sm">
            <div className="flex items-center gap-2">
              <Clock size={16} className={mutedIconClass} />
              <span className="font-medium text-foreground/85">Pendiente</span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle size={16} className={statusIconClass} />
              <span className="font-medium text-foreground/85">Conciliado Manual</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="relative inline-block">
                <CheckCircle size={16} className={statusIconClass} />
                <Link2
                  size={8}
                  className="absolute -bottom-px -right-px rounded-full bg-card text-primary"
                />
              </div>
              ...
              <span
                className="rounded border border-cyan-200 bg-primary/10 px-2 py-0.5 text-xs font-semibold text-primary"
              >
                Resaltado
              </span>
            </div>
                ...
                <tbody>
                  {movimientosSistemaConciliados.map((mov) => (
                    <tr
                      ...
                      className={cn(
                        'border-b border-border border-l-4 border-l-cyan-500 bg-primary/10/70 transition duration-200',
                      ...
                      <td className={tdClass}>
                        <span
                          className={cn(
                            ...
                            mov.es_extracto ? 'bg-primary/10 text-primary' : 'bg-primary/10 text-primary'
                          )}
                        >
