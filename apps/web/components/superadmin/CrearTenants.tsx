...
export default function CrearTenants() {
  ...
  const handleDemoTenant = (tenant: Tenant) => {
    setSelectedTenant(tenant)
  ...
  const DemoTenantModal = useMemo(
    () =>
      dynamic<{ tenant: Tenant; onClose: () => void; onSuccess: () => void }>(
        () => import('./components/DemoTenantModal').then(mod => mod.default),
        { ssr: false }
      ),
    []
  )
  ...
  return (
    <>
      ...
      {demoModalOpen && selectedTenant && (
        <DemoTenantModal
          tenant={selectedTenant}
          onClose={() => { setDemoModalOpen(false); setSelectedTenant(null) }}
          onSuccess={fetchTenants}
        />
      )}
    </>
