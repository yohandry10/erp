          ...
          <CardContent>
            <ul className="space-y-2 text-sm">
              ...
              {bucketTotales.filter((bucket) => bucket.monto > 0).length === 0 && (
                <li className="text-slate-100/80">Sin montos significativos</li>
              )}
            </ul>
          </CardContent>
        </Card>
          ...
          <table className="min-w-full divide-y divide-slate-200 text-sm">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-3 py-2 text-right font-medium text-slate-600">% del total</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {bucketTotales.map((bucket) => (
                <tr key={bucket.nombre} className="bg-white">
                  <td className="px-3 py-2 text-slate-700 font-medium">{bucket.nombre}</td>
                  <td className="px-3 py-2 text-slate-500">{bucket.rango}</td>
            ...
            <table className="min-w-full divide-y divide-slate-200 text-sm">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="px-3 py-2 text-right font-medium text-slate-600">% cartera</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {data.saldoPorCliente.length === 0 ? (
                  ...
                  data.saldoPorCliente.slice(0, 10).map((cliente) => (
                    <tr key={cliente.cliente} className="bg-white">
                      <td className="px-3 py-2 text-slate-600">{cliente.cliente}</td>
                      <td className="px-3 py-2 text-right text-slate-600 font-semibold">
            ...
            <table className="min-w-full divide-y divide-slate-200 text-sm">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left font-medium text-slate-600">Cliente</th>
                  <th className="px-3 py-2 text-right font-medium text-slate-600">Monto</th>
                  <th className="px-3 py-2 text-right font-medium text-slate-600">Días mora</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {data.cuentasCriticas.length === 0 ? (
                  ...
                  data.cuentasCriticas.map((cuenta) => (
                    <tr key={cuenta.id} className="bg-white">
                      <td className="px-3 py-2 text-slate-700 font-medium">{cuenta.documento}</td>
                      <td className="px-3 py-2 text-slate-600">
                        <div>{cuenta.cliente}</div>
                        <div className="text-xs text-slate-400">{cuenta.cliente_documento ?? '—'}</div>
                      </td>
                      <td className="px-3 py-2 text-right text-slate-600 font-semibold">
                      ...
                      <td className="px-3 py-2 text-slate-500">{cuenta.estado}</td>
                    </tr>
                  ))
                )}
              </tbody>
