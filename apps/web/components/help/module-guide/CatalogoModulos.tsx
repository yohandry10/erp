...
export function CatalogoModulos() {
  ...
  const grupos = useMemo(() => agruparGuias(country.paisCodigo), [country.paisCodigo])
    ...
    <div className="flex flex-col gap-5">
      {grupos.map((grupo) => (
        <section key={grupo.area}>
          <p className="m-0 mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{grupo.area}</p>
            ...
            {grupo.modulos.map(({ ruta, guia }) => {
              ...
              return (
                ...
                >
                  <button
                    type="button"
                    data-testid="module-guide-entry"
                  ...
                  >
                    ...
                    <ChevronDown className={`h-4 w-4 shrink-0 text-muted-foreground transition-transform ${activo ? 'rotate-180' : ''}`} />
                  </button>
                  ...
                  {activo && (
                    <div className="flex flex-col gap-3 border-t border-cyan-400/15 px-3 py-3 group-data-[erp-theme=light]/dashboard:border-border">
                      ...
                      <div>
                        <p className="m-0 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Qué puedes hacer aquí</p>
