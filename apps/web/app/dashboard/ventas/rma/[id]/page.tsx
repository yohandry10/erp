...
export default function RmaDetailPage() {
  ...
  const {
    status: demoStatus,
    isDemoTenant,
    loading: demoStatusLoading,
    error: demoStatusError,
  } = useDemoStatus()
    ...
    <div className="mx-auto w-full max-w-[1400px] space-y-6 p-4 md:p-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_380px]">
        ...
        <aside className="space-y-4">
          ...
          {rma.estado === 'RECIBIDA' && country.paisCodigo === 'CO' && rma.nota_credito_cpe_id && <ActionCard icon={FileCheck2} title="Nota crédito DIAN 91 pendiente" description="La devolución quedó enlazada con su nota fiscal. La CxC todavía no cambia: el cierre ocurre únicamente cuando DIAN acepte la nota."><Info label="Documento NC" value={rma.nota_credito_documento_id ?? '—'} /><Info label="CPE DIAN 91" value={rma.nota_credito_cpe_id} /><Button asChild className="mt-4 w-full"><Link href="/dashboard/cpe">Firmar, enviar y consultar en CPE</Link></Button></ActionCard>}

          {rma.estado === 'RECIBIDA' && country.paisCodigo === 'CO' && !rma.nota_credito_cpe_id && !demoStatusLoading && (!demoStatus || demoStatusError) && <ActionCard icon={ShieldAlert} title="Estado fiscal no disponible" description="No se pudo confirmar si esta cuenta es demo o real."><p className="rounded-lg bg-amber-500/10 p-3 text-xs text-amber-800 dark:text-amber-200">Por seguridad no se habilita la Nota Crédito DIAN 91. Recarga la página o vuelve a iniciar sesión; no se enviará ninguna operación mientras el estado siga sin verificarse.</p></ActionCard>}

          {rma.estado === 'RECIBIDA' && country.paisCodigo === 'CO' && !rma.nota_credito_cpe_id && !demoStatusLoading && demoStatus && !demoStatusError && isDemoTenant && <ActionCard icon={FileCheck2} title="Nota crédito DIAN 91 no disponible en demo" description="La devolución física sí queda registrada, pero una demo no posee una factura real aceptada por DIAN que pueda acreditarse."><p className="rounded-lg bg-amber-500/10 p-3 text-xs text-amber-800 dark:text-amber-200">No se simulará aceptación fiscal ni se reducirá la CxC. Convierte la cuenta y completa certificado, software, TestSet y resolución DIAN.</p></ActionCard>}

          {rma.estado === 'RECIBIDA' && country.paisCodigo === 'CO' && !rma.nota_credito_cpe_id && !demoStatusLoading && demoStatus && !demoStatusError && !isDemoTenant && <ActionCard icon={FileCheck2} title="Crear Nota Crédito DIAN 91" description="El servidor tomará exclusivamente las líneas y cantidades ya recibidas en esta RMA. La CxC sólo cambia después de la aceptación DIAN."><label className="block"><span className="mb-1 block text-xs font-medium">Motivo</span><Input value={creditReason} onChange={(event) => setCreditReason(event.target.value)} /></label><p className="mt-3 rounded-lg bg-amber-500/10 p-3 text-xs text-amber-800 dark:text-amber-200">Las bases e impuestos se recalculan desde la factura origen; no se aceptan importes ni líneas escritos por el navegador.</p><Button className="mt-4 w-full" disabled={saving || creditReason.trim().length < 3} onClick={() => void run('credit-note-dian', `/api/ventas/rma/${rma.id}/nota-credito`, { motivo: creditReason }, 'Nota DIAN 91 creada y enlazada')}>Crear NC 91 por líneas recibidas</Button></ActionCard>}
          ...
          {rma.estado === 'CERRADA' && <ActionCard icon={BadgeDollarSign} title="Cierre financiero" description="La factura original permanece vigente; sólo se acreditaron las líneas devueltas."><Info label="Documento NC" value={rma.nota_credito_documento_id ?? '—'} /><Info label={country.paisCodigo === 'CO' ? 'CPE DIAN 91' : 'CPE 07'} value={rma.nota_credito_cpe_id ?? '—'} />{rma.saldo_favor && <div className="mt-4 rounded-xl bg-emerald-500/10 p-4"><p className="text-xs font-bold uppercase text-emerald-700 dark:text-emerald-300">Saldo a favor disponible</p><p className="mt-1 text-2xl font-black text-emerald-700 dark:text-emerald-300">{formatCurrency(Number(rma.saldo_favor.monto_disponible), rma.saldo_favor.moneda)}</p><Button asChild variant="link" className="mt-1 h-auto p-0"><Link href="/dashboard/ventas/rma">Gestionar aplicación o reembolso</Link></Button></div>}</ActionCard>}
