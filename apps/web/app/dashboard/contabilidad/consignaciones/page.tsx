    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-muted/50 to-background p-4 text-foreground">
      <div className="mx-auto max-w-[1500px] space-y-4">
        ...
        <Card className="min-w-0 border-cyan-400/20 bg-card/70">
          <CardHeader className="gap-4 border-b border-border sm:flex-row sm:items-center sm:justify-between"><div><CardTitle className="text-base">Registro operativo</CardTitle><p className="mt-1 text-sm text-muted-foreground">{items.length} consignación(es) · {formatMoney(total)}</p></div><select value={estado} onChange={(e) => setEstado(e.target.value)} className="h-10 min-w-0 rounded-lg border border-border bg-background px-3 text-sm"><option value="">Todos los estados</option>{estados.map((value) => <option key={value} value={value}>{value}</option>)}</select></CardHeader>
