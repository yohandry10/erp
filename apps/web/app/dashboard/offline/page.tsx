    ...
    <div className="mx-auto max-w-7xl space-y-6 pb-10">
      ...
      {(conflictCount > 0 || snapshotSummary) && (
        <div className="grid gap-4 md:grid-cols-2">
          {conflictCount > 0 && (
            <div className={panelClass}>
              <div className="flex items-center gap-2 text-sm font-semibold text-amber-200 group-data-[erp-theme=light]/dashboard:text-amber-700">
                <AlertTriangle className="h-4 w-4" />
                Conflictos detectados
              </div>
              <div className="mt-2 text-sm text-slate-300 group-data-[erp-theme=light]/dashboard:text-slate-600">
          ...
          {snapshotSummary && (
            <div className={panelClass}>
              <div className="flex items-center gap-2 text-sm font-semibold text-cyan-100 group-data-[erp-theme=light]/dashboard:text-slate-700">
                <Database className="h-4 w-4" />
                Cache local
              </div>
              <div className="mt-2 text-sm text-slate-300 group-data-[erp-theme=light]/dashboard:text-slate-600">{snapshotSummary}</div>
            </div>
          )}
        </div>
      )}
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>
              {queue.length === 0 ? (
                <tr>
                  <td colSpan={9} className="px-3 py-8 text-center text-slate-400">No hay operaciones offline.</td>
                </tr>
              ) : queue.map((item) => (
                <tr key={item.id} className="border-b border-cyan-400/10 group-data-[erp-theme=light]/dashboard:border-slate-100">
                  <td className="px-3 py-3">
                    ...
                    {isConflict(item) && <div className="mt-1 text-xs font-semibold text-amber-200 group-data-[erp-theme=light]/dashboard:text-amber-700">Conflicto</div>}
                  </td>
                  ...
                  <td className="max-w-[180px] px-3 py-3 font-mono text-xs">
                    ...
                    <div className="truncate text-slate-500">{item.user_id || '-'}</div>
                  </td>
                  ...
                  <td className="max-w-[320px] px-3 py-3 text-amber-200 group-data-[erp-theme=light]/dashboard:text-amber-700">
                    <details>
                      <summary className="cursor-pointer truncate">{item.last_error || previewPayload(item.body) || '-'}</summary>
                      <pre className="mt-2 max-h-40 max-w-[360px] overflow-auto whitespace-pre-wrap rounded border border-slate-500/20 bg-slate-950/80 p-2 text-[11px] text-slate-200 group-data-[erp-theme=light]/dashboard:bg-slate-50 group-data-[erp-theme=light]/dashboard:text-slate-700">
                        {formatDetails(item)}
                      </pre>
                    </details>
                  </td>
      ...
      <div className={panelClass}>
        <div className="mb-4 flex items-center justify-between">
          ...
          <span className="text-sm text-slate-400 group-data-[erp-theme=light]/dashboard:text-slate-500">{mappings.length} total</span>
        </div>
        <div className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead className="border-b border-cyan-400/10 text-xs uppercase text-slate-400 group-data-[erp-theme=light]/dashboard:border-slate-200">
            ...
            <tbody>
              {mappings.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-3 py-8 text-center text-slate-400">Aun no hay mapeos local-remoto.</td>
                </tr>
              ) : mappings.map((item) => (
                <tr key={`${item.local_id}-${item.remote_id}`} className="border-b border-cyan-400/10 group-data-[erp-theme=light]/dashboard:border-slate-100">
