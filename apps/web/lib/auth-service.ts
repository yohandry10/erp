...
class AuthService {
  ...
  constructor() {
    // Cargar sesión del sessionStorage al iniciar
  ...
  private loadSession() {
    try {
      console.log('🔄 [AuthService] Cargando sesión desde sessionStorage...');
      
      // Diagnóstico completo de sessionStorage
      if (typeof window !== 'undefined' && typeof sessionStorage !== 'undefined') {
        const allKeys = Object.keys(sessionStorage);
        console.log('🔍 [AuthService] Diagnóstico sessionStorage:', {
          allKeys: allKeys,
          hasAccessToken: sessionStorage.getItem('access_token') !== null,
          hasUser: sessionStorage.getItem('user') !== null,
      ...
      if (token) {
        ...
        try {
          ...
          // Crear usuario desde el JWT, no desde sessionStorage
          const user = {
            id: payload.sub,
            email: payload.email,
          ...
          // Actualizar sessionStorage con datos correctos del JWT
        ...
        console.log('ℹ️ [AuthService] No hay token en sessionStorage');
        ...
        console.log('   3. El sessionStorage fue limpiado manualmente o por el navegador');
      ...
      if (error instanceof Error && error.message.includes('sessionStorage')) {
        console.error('❌ [AuthService] Error crítico con sessionStorage - puede estar deshabilitado o bloqueado');
        this.clearSession();
      }
    }
  }
  ...
  private saveSession(session: Session) {
    ...
    if (typeof window !== 'undefined' && typeof sessionStorage !== 'undefined') {
      try {
        ...
        // ✅ CRÍTICO: Forzar sincronización del sessionStorage
        // Algunos navegadores hacen esto de forma asíncrona
        const forceSync = sessionStorage.getItem('access_token');
        ...
        if (!savedToken || savedToken !== session.access_token) {
          console.error('❌ [AuthService] CRÍTICO: Token no se guardó correctamente en sessionStorage');
          ...
          throw new Error('Failed to save token to sessionStorage');
        }
        ...
        console.error('❌ [AuthService] Error guardando en sessionStorage:', error);
        // Re-throw para que el login maneje el error
        throw error;
      }
    } else {
      console.error('❌ [AuthService] sessionStorage no disponible - ejecutando en servidor?');
  ...
  async getSession(): Promise<{ data: { session: Session | null }; error: Error | null }> {
    // Si no hay sesión en memoria, intentar cargar desde sessionStorage
    if (!this.session && typeof window !== 'undefined') {
      console.log('🔄 [AuthService] getSession: No hay sesión en memoria, recargando...');
      this.loadSession();
    }
    
    // Verificar que el token aún existe en sessionStorage
    if (this.session && typeof window !== 'undefined') {
      const storedToken = sessionStorage.getItem('access_token');
      if (!storedToken || storedToken !== this.session.access_token) {
        console.warn('⚠️ [AuthService] Token en memoria no coincide con sessionStorage, recargando...');
