-- Cierra la incompatibilidad entre el modelo legacy de plantillas (cuenta_codigo)
-- y el modelo operativo de la migracion 385 (cuenta_id + debe/haber). Tambien
-- concentra cabecera y lineas en una sola transaccion PostgreSQL.
    ...
    AND (
      ...
      OR (
        ...
        AND (
          ...
          OR (cuenta_codigo IS NOT NULL AND btrim(cuenta_codigo) <> '')
        )
        AND (
          cuenta_id IS NULL
          OR (
...
COMMENT ON FUNCTION public.guardar_plantilla_con_detalles_tx(uuid, text, uuid, jsonb, jsonb) IS
  'Crea o actualiza cabecera y lineas de una plantilla contable en una sola transaccion; valida cuentas y centros dentro del tenant.';
