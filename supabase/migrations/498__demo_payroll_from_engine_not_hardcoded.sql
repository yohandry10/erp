-- 498__demo_payroll_from_engine_not_hardcoded.sql
--
-- El seed de la demo escribía la planilla con los importes puestos a mano, y el
-- del trabajador afiliado a AFP estaba mal: anotaba 416 sobre una remuneración de
-- 3 200, que es exactamente el 13 % de la ONP y no el 12,92 % que corresponde a
-- una afiliación AFP (10 % de aporte + 1,55 % de comisión de flujo + 1,37 % de
-- prima de seguro). El contrato de ese empleado sí dice `AFP`, así que la demo se
-- contradecía a sí misma.
--
-- Es el mismo defecto que cerró la migración 497 con la venta POS: dato derivado
-- escrito a mano en lugar de producido por el motor. Y es un defecto que reaparece
-- solo, porque cualquier cambio de tasa o de regla vuelve a desalinear el número.
-- De hecho acaba de cambiar: la retención de quinta categoría ahora sigue el
-- procedimiento del artículo 40 del Reglamento de la LIR, con divisor por mes y
-- descuento de lo ya retenido, así que el importe correcto ya no es el de ayer.
--
-- No existe un writer SQL de planilla al que delegar el cálculo —vive en el
-- servicio de RR. HH.—, de modo que la alternativa honesta no es corregir el
-- número sino dejar de afirmarlo. La planilla demo nace en borrador y sin líneas
-- por empleado; el usuario pulsa «Calcular» y ve lo que produce el motor real, que
-- es además mejor demostración del producto. El readiness de la demo sólo exige
-- que la planilla exista, no que traiga líneas precalculadas.
--
-- La migración se limita a redefinir `app.hydrate_demo_business_sample_tx`. No
-- toca datos existentes: los tenants demo ya creados conservan su planilla.
...
BEGIN
  ...
  -- La planilla demo nace en borrador y sin líneas por empleado.
  --
  -- Antes traía los importes escritos a mano, y estaban mal: al trabajador con
  -- contrato AFP le anotaba 416 sobre 3 200, que es exactamente el 13 % de la ONP
  -- y no el 12,92 % que corresponde a una afiliación AFP. Era el mismo defecto que
  -- la venta POS: dato derivado escrito a mano en vez de producido por el motor,
  -- que además vuelve a desalinearse cada vez que cambia una tasa o una regla —y
  -- acaba de cambiar, porque la retención de quinta ahora sigue el artículo 40.
  --
  -- No existe un writer SQL de planilla al que delegar, así que la alternativa
  -- honesta es no afirmar un cálculo: se deja el borrador para que el usuario pulse
  -- «Calcular» y vea lo que produce el motor real. El readiness de la demo sólo
  -- exige que exista la planilla, no que tenga líneas.
  INSERT INTO public.planillas (
    ...
    'borrador', 'pendiente', 0, 0, 0, 0,
    'PE', 'PEN', '{"source":"demo_business_seed_v1","calculo":"motor"}'::jsonb
  );
