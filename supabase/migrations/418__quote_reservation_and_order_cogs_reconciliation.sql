...
-- Ambos RPC aceptan tenant_id y son SECURITY DEFINER. Exponerlos a anon o
-- authenticated permitiría saltar RLS eligiendo otro tenant en el payload.
-- Sólo el backend con service_role coordina esta transición de inventario.
...
revoke all on function public.convertir_cotizacion_a_pedido(uuid, uuid, uuid, text)
  from public, anon, authenticated;
grant execute on function public.liberar_stock_cotizacion(uuid, uuid)
  to service_role;
grant execute on function public.convertir_cotizacion_a_pedido(uuid, uuid, uuid, text)
  to service_role;
...
drop table if exists pg_temp.migration_418_order_costs;
create temporary table migration_418_order_costs as
...
from migration_418_order_costs;
...
from (
  ...
  where d.asiento_id in (select asiento_id from migration_418_order_costs)
...
begin
  if exists (
    ...
    where id in (select asiento_id from migration_418_order_costs)
      and abs(coalesce(total_debe,0)-coalesce(total_haber,0)) >= 0.01
  ) then raise exception 'migration 418 produjo un asiento desbalanceado'; end if;
end
$$;
