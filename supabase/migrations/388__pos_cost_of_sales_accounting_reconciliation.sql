...
drop table if exists pg_temp.migration_388_pos_costs;
create temporary table migration_388_pos_costs as
select
  a.id as asiento_id,
  a.tenant_id,
...
from migration_388_pos_costs t
where not exists (
  select 1
  from public.detalle_asientos d
  join public.plan_cuentas p on p.id = d.cuenta_id
...
from (
  ...
  from public.detalle_asientos d
  where d.asiento_id in (select asiento_id from migration_388_pos_costs)
...
begin
  if exists (
    select 1
    from public.asientos_contables a
    where a.id in (select asiento_id from migration_388_pos_costs)
      and abs(coalesce(a.total_debe, 0) - coalesce(a.total_haber, 0)) >= 0.01
  ) then
    raise exception 'migration 388 produjo un asiento POS desbalanceado';
  end if;
end
$$;
