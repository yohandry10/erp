-- ============================================================================
-- 327__pos_full_transaction_performance.sql
-- POS: registra venta, detalle, pagos, stock, caja y outbox en una sola
-- transacción para evitar latencia por múltiples roundtrips desde la API.
-- ============================================================================

BEGIN;

CREATE UNIQUE INDEX IF NOT EXISTS ux_ventas_pos_tenant_idempotency_key_runtime
  ON public.ventas_pos (tenant_id, idempotency_key)
...
BEGIN
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(COALESCE(p_items, '[]'::jsonb)) AS t(value) LOOP
    ...
    v_impuesto_item := round(v_subtotal_item * v_tasa_igv, 2);
