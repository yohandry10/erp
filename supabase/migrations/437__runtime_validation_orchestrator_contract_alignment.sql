-- Alinea los validadores reintroducidos después del orquestador 296.

BEGIN;

ALTER FUNCTION public.acquire_job_lock(text, integer)
  SET search_path = public, app, pg_temp;
ALTER FUNCTION public.release_job_lock(text)
  SET search_path = public, app, pg_temp;
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      IF v_fn.proname = 'validar_deployment_environment_runtime' THEN
        FOR v_row IN
          SELECT
            'validar_deployment_environment_runtime'::text AS pack_name,
            x.check_name,
            x.ok,
            x.detail
          FROM public.validar_deployment_environment_runtime(
            'PROD',
            'wypnbcptofqdmoynlonq'
          ) AS x
        LOOP
          IF (NOT p_only_failed) OR (v_row.ok IS NOT TRUE) THEN
      ...
      ELSIF v_fn.proname = 'validar_tesoreria_caja_bancos_runtime' THEN
        FOR v_row IN
          SELECT
            'validar_tesoreria_caja_bancos_runtime'::text AS pack_name,
            x.control AS check_name,
            x.estado = 'OK' AS ok,
            format('severidad=%s total=%s %s', x.severidad, x.total, x.detalle) AS detail
          FROM public.validar_tesoreria_caja_bancos_runtime(v_tenant_id) AS x
        LOOP
          IF (NOT p_only_failed) OR (v_row.ok IS NOT TRUE) THEN
            pack_name := v_row.pack_name;
            check_name := v_row.check_name;
