...
-- Implementación interna/trigger con SECURITY DEFINER. No debe heredar el
-- EXECUTE implícito de PUBLIC, aunque el esquema app tampoco se exponga.
REVOKE ALL ON FUNCTION app.normalizar_identidad_conversion_demo() FROM PUBLIC;
REVOKE ALL ON FUNCTION app.promover_admin_al_convertir_demo() FROM PUBLIC;
REVOKE ALL ON FUNCTION app.completar_conversion_demo(
  uuid, text, text, text, text, text, text, boolean, uuid, text, text
