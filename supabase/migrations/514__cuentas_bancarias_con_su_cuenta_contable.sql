-- Migracion 514: las cuentas bancarias sembradas pueden registrar movimientos.
--
-- La semilla creaba la cuenta del banco **sin cuenta contable asociada**, y
-- `assert_postable_account_457` exige que la tenga: cualquier movimiento
-- bancario --el pan de cada dia-- fallaba con
-- `BANK_LEDGER_ACCOUNT_NOT_POSTABLE_IN_TENANT`, que ademas no dice que hay que
-- hacer. Comprobado contra produccion: las 67 cuentas bancarias que existen lo
-- tenian a nulo, o sea **todas**.
--
-- La cuenta correcta es la corriente operativa (1041 del PCGE). No sirve la
-- 1042, que es la de detracciones y tiene su propio saldo, ni la 10, que es de
-- resumen. Donde no exista se crea, porque el catalogo runtime tampoco la traia.
--
-- Se corrigen las tres cosas: la funcion que siembra, el catalogo de cuentas
-- --en el codigo del API-- y las filas que ya estan escritas.
...
BEGIN
  ...
  -- La cuenta del plan a la que apunta el banco. Sin ella
  -- `assert_postable_account_457` rechaza cualquier movimiento bancario, asi
  -- que la cuenta sembrada nacia inservible. No vale la 1042, que es la de
  -- detracciones, ni la 10, que es de resumen.
  SELECT id INTO v_bank_ledger_id
  FROM public.plan_cuentas
  WHERE tenant_id = p_tenant_id AND codigo IN ('1041', '104')
    AND COALESCE(acepta_movimiento, false)
    AND upper(COALESCE(estado::text, 'ACTIVO')) = 'ACTIVO'
  ORDER BY codigo DESC
  LIMIT 1;

  IF v_bank_ledger_id IS NULL THEN
    INSERT INTO public.plan_cuentas (
      tenant_id, codigo, nombre, tipo, tipo_cuenta, nivel, acepta_movimiento, estado, activo
    ) VALUES (
      p_tenant_id, '1041', 'Cuentas corrientes operativas', 'ACTIVO', 'ACTIVO', 4, true, 'ACTIVO', true
    )
...
-- Cuentas bancarias ya sembradas. A cada contribuyente que tenga una cuenta
-- bancaria sin cuenta contable se le resuelve la corriente operativa, creandola
-- si no la tiene.
INSERT INTO public.plan_cuentas (tenant_id, codigo, nombre, tipo, tipo_cuenta, nivel, acepta_movimiento, estado, activo)
SELECT DISTINCT cb.tenant_id, '1041', 'Cuentas corrientes operativas', 'ACTIVO', 'ACTIVO', 4, true, 'ACTIVO', true
FROM public.cuentas_bancarias cb
WHERE cb.cuenta_contable_id IS NULL
  AND NOT EXISTS (
    SELECT 1 FROM public.plan_cuentas pc
    WHERE pc.tenant_id = cb.tenant_id AND pc.codigo IN ('1041', '104')
      AND COALESCE(pc.acepta_movimiento, false)
      AND upper(COALESCE(pc.estado::text, 'ACTIVO')) = 'ACTIVO'
  );

UPDATE public.cuentas_bancarias cb
SET cuenta_contable_id = (
      SELECT p.id FROM public.plan_cuentas p
      WHERE p.tenant_id = cb.tenant_id AND p.codigo IN ('1041', '104')
        AND COALESCE(p.acepta_movimiento, false)
        AND upper(COALESCE(p.estado::text, 'ACTIVO')) = 'ACTIVO'
      ORDER BY p.codigo DESC
      LIMIT 1),
    updated_at = now()
WHERE cb.cuenta_contable_id IS NULL
  -- Se exige que exista antes de escribirla: dejarla nula seria como estaba, y
  -- el verificador lo cazaria, pero mejor no tocar la fila para nada.
  AND EXISTS (
    SELECT 1 FROM public.plan_cuentas p
    WHERE p.tenant_id = cb.tenant_id AND p.codigo IN ('1041', '104')
      AND COALESCE(p.acepta_movimiento, false)
      AND upper(COALESCE(p.estado::text, 'ACTIVO')) = 'ACTIVO'
  );
