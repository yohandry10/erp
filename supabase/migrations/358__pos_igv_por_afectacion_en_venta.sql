-- ============================================================================
-- 358__pos_igv_por_afectacion_en_venta.sql
-- La venta POS se liquidaba con la tasa de IGV plana sobre todo el subtotal, sin
-- mirar la afectacion del Catalogo 07 de cada item. Una boleta con bienes
-- exonerados quedaba registrada y cobrada con IGV mientras el CPE se emitia
-- correctamente con base exonerada e IGV 0: el cliente pagaba un importe
-- distinto al de su comprobante.
--
-- El backend ya calcula el desglose por afectacion y ahora reparte ese IGV entre
-- los items. Las RPC lo suman en vez de recalcularlo. Si ningun item trae 'igv'
-- se conserva la tasa plana como compatibilidad para no bloquear la venta.
--
-- Solo cambia la liquidacion del impuesto: firma, numeracion, stock, caja,
-- outbox e idempotencia quedan identicos.
-- ============================================================================
...
IS 'Registra la venta POS liquidando el IGV que trae cada item segun su afectacion (Catalogo 07); tasa plana solo como compatibilidad si ningun item lo declara.';
...
COMMENT ON FUNCTION public.pos_registrar_venta_tx(uuid, uuid, uuid, text, text, text, jsonb, text, uuid, text, numeric, text, jsonb)
IS 'Registra la venta POS liquidando el IGV que trae cada item segun su afectacion (Catalogo 07); tasa plana solo como compatibilidad si ningun item lo declara.';
