# E2E Test: Aplicar Pago a CxP (con login automático)
# Este script obtiene un token válido y prueba el flujo completo de aplicar un pago a una cuenta por pagar
...
try {
    ...
    Write-Host "  Verifica que el servidor esté corriendo y las credenciales sean correctas" -ForegroundColor Yellow
    exit 1
}
...
try {
    ...
    if ($response.data -and $response.data.Count -gt 0) {
        ...
        Write-Host "  Tip: Crea una CxP primero desde el sistema" -ForegroundColor Yellow
    }
} catch {
