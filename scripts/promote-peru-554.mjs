...
assert.equal(rehearsal.historyBefore, 553);
assert.equal(rehearsal.historyAfter, 554);
...
assert.equal(Math.max(...versions), 554);
...
const env = { ...process.env, PGHOST: connection.hostname, PGPORT: connection.port || '5432',
  ...
  PGAPPNAME: 'erp-peru-schema-554-promotion', PGOPTIONS: '-c statement_timeout=120000 -c lock_timeout=10000' };
...
function sql(input, step) {
  ...
  fs.writeFileSync(path.join(root, 'artifacts/db-backups', `promotion-554-${runId}-${step}.log`), result.stderr || '');
...
SELECT json_build_object(
  ...
  'migrationRunsAcl',has_table_privilege('service_role','public.migration_runs','INSERT'),
  'migrationRowsAcl',has_table_privilege('service_role','public.migration_run_rows','INSERT')
);
COMMIT;`, expected === 553 ? 'before' : 'after'));
try {
  report.before = state(553);
  assert.equal(report.before.project, project);
  assert.equal(report.before.environment, 'PROD');
  assert.equal(report.before.schema, 553, 'No reaplicar 554');
  assert.equal(report.before.ready, true);
  report.remoteWrites = true; // Respuesta perdida del commit = resultado incierto.
  sql(bundle, 'apply');
  report.after = state(554);
  assert.equal(report.after.project, project);
  assert.equal(report.after.schema, 554);
  assert.equal(report.after.ready, true);
  assert.equal(report.after.migrationRunsAcl, true);
  assert.equal(report.after.migrationRowsAcl, true);
  report.success = true;
} finally {
  report.completedAt = new Date().toISOString();
  fs.writeFileSync(path.join(root, 'artifacts', `peru-554-promotion-${runId}.json`), JSON.stringify(report, null, 2));
}
