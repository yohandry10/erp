...
assert.equal(process.argv.length, 3, 'Uso: node scripts/rehearse-peru-555-local.mjs artifacts/peru-prod-backup-<id>.json');
...
const containerName = `erp-peru-555-rehearsal-${runId}`;
...
try {
  ...
  container = docker(['run', '--rm', '--detach', '--name', containerName,
    '--label', `erp.peru.555.rehearsal=${runId}`, '--network', 'none',
  ...
  assert.equal(before.history, 554, 'El respaldo debe estar exactamente en 554');
  step = 'rollback atómico 555';
  ...
  step = 'aplicar 555 en copia aislada';
  sql(bundle);
  const after = snapshot();
  assert.equal(after.history, 555);
  assert.equal(after.branchAcl, true);
  assert.equal(after.assignmentAcl, true);
  assert.equal(after.permissions, before.permissions);
  assert.equal(after.rolePermissions, before.rolePermissions);
