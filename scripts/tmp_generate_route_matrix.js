...
function isHttpMethod(name) {
  return ['Get','Post','Put','Delete','Patch','Options','Head','All'].includes(name);
}
...
const controllerFiles = [
  ...walk(path.join(TARGET, 'modules')).filter((file) => file.endsWith('.controller.ts')),
  ...walk(path.join(TARGET, 'shared')).filter((file) => file.endsWith('.controller.ts')),
...
for (const file of controllerFiles) {
  ...
  const visit = (node) => {
    ...
    const controllerDecorator = classDecorators.find(d => decoratorName(d) === 'Controller');
    const controllerBase = controllerDecorator && ts.isCallExpression(controllerDecorator) && controllerDecorator.arguments.length
    ...
    const classHasPublic = classDecorators.some(d => decoratorName(d) === 'Public');
    ...
    for (const member of node.members) {
      ...
      const httpDecorators = methodDecorators.filter(d => isHttpMethod(decoratorName(d)));
      ...
      const hasPublic = classHasPublic || methodDecorators.some(d => decoratorName(d) === 'Public');
      const hasPermission = methodDecorators.some(d => decoratorName(d) === 'RequirePermission');
      const permDecorator = methodDecorators.find(d => decoratorName(d) === 'RequirePermission');
      const permission = permDecorator ? extractPermissionText(permDecorator) : '';

      const classHasJwt = hasGuard(classGuards, 'JwtAuthGuard') || hasGuard(classGuards, 'AuthGuard');
      const methodHasJwt = hasGuard(methodGuards, 'JwtAuthGuard') || hasGuard(methodGuards, 'AuthGuard');
      const classHasPermissionGuard = hasGuard(classGuards, 'PermissionGuard');
      const methodHasPermissionGuard = hasGuard(methodGuards, 'PermissionGuard');
      ...
      const hasPermissionGuard = classHasPermissionGuard || methodHasPermissionGuard;
      ...
      } else if (!hasJwt && !hasPermissionGuard && !hasSuperAdmin && !hasPublic) {
      ...
      for (const decorator of httpDecorators) {
        const methodPath = ts.isCallExpression(decorator) && decorator.arguments.length
...
fs.writeFileSync('docs/security/route-access-matrix.md', md.join('\n'), 'utf8');
