...
| Migración/importación de maestros y exportaciones | `artifacts/peru-integrated-20260923092617881-10028`: primer ADMIN no demo previsualiza clientes/proveedores, ejecuta dry-run sin escritura, importa archivo mixto con error por fila, consulta lote, reintenta sin duplicar y oculta lote ajeno con 404. Navegador descarga plantilla, bloquea CSV inválido/503, reintenta, importa, busca y exporta ambas filas. La 554 se promovió y el runtime #113 está desplegado (`artifacts/peru-production-verification-after-113-20260923.json`). | Publicar corrección de revocación temprana de Blob URL; importaciones CxC/CxP/saldos/stock/comprobantes; actualización por CSV, roles diferenciados y archivos grandes | Clientes/proveedores OK local API+navegador; resto pendiente |
